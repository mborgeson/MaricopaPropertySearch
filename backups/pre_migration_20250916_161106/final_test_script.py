...
def test_all_methods():
    ...
    try:
        ...
        if result:
            ...
            payment_status = scraped_data.get("current_tax", {}).get(
                "payment_status", "N/A"
            )
    ...
    try:
        ...
        print(
            f"   PASS: Invalid APN correctly handled with exception: {type(e).__name__}"
        )
        results["error_handling"] = True
    print()
    ...
    if not results.get("property_details"):
        ...
        for backup_apn in backup_apns:
            try:
                ...
                if result and (
                    result.get("owner_name") or result.get("property_address")
                ):
