...
def check_tables_and_data(test_apn="11727002"):
    ...
    try:
        ...
        if sales_count > 0:
            cursor.execute(
                "SELECT sale_date, sale_price FROM sales_history WHERE apn = %s ORDER BY sale_date DESC LIMIT 3",
                (test_apn,),
            )
            ...
            for record in sales_records:
                print(
                    f"     - {record['sale_date']}: ${record.get('sale_price', 'N/A')}"
                )
    ...
    try:
        ...
        if tax_history:
            ...
            for record in tax_history[:2]:
                print(
                    f"     - Year {record.get('tax_year')}: ${record.get('assessed_value', 'N/A')}"
                )
        ...
        if sales_history:
            ...
            for record in sales_history[:2]:
                print(
                    f"     - {record.get('sale_date')}: ${record.get('sale_price', 'N/A')}"
                )
...
def test_insert_sample_data(test_apn="11727002"):
    ...
    try:
        ...
        if tax_inserted or sales_inserted:
            ...
            print(
                f"   After insertion - Tax: {len(tax_history)}, Sales: {len(sales_history)}"
            )
...
def cleanup_test_data(test_apn):
    ...
    try:
        ...
        cursor.execute(
            "DELETE FROM tax_history WHERE apn = %s AND payment_status = 'TEST_DATA'",
            (test_apn,),
        )
        cursor.execute(
            "DELETE FROM sales_history WHERE apn = %s AND recording_number = 'TEST123'",
            (test_apn,),
        )
