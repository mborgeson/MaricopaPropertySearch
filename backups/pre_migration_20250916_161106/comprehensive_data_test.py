...
class ComprehensiveDataTest:
    ...
    def setup(self):
        ...
        try:
            ...
            if self.db_manager.test_connection():
                ...
                logger.warning(
                    "⚠ Database connection failed - some tests may be limited"
                )
    ...
    def test_property_details(self):
        ...
        try:
            ...
            if property_data:
                ...
                logger.info(
                    f"   Assessed Value: ${property_data.get('assessed_value', 'N/A'):,}"
    ...
    def test_tax_information(self):
        ...
        try:
            ...
            if tax_data:
                ...
                logger.info(
                    f"   Tax Amount: ${tax_data.get('tax_amount', 'N/A'):,}"
                    if tax_data.get("tax_amount")
                    else "   Tax Amount: N/A"
                )
                logger.info(f"   Exemptions: {tax_data.get('exemptions', 'N/A')}")
                logger.info(
                    f"   Payment Status: {tax_data.get('payment_status', 'N/A')}"
                )
        ...
        try:
            ...
            if sales_data:
                ...
                for i, sale in enumerate(sales_data[:3]):  # Show first 3 sales
                    ...
                    logger.info(
                        f"     Price: ${sale.get('sale_price', 'N/A'):,}"
    ...
    def test_property_documents(self):
        ...
        try:
            ...
            if documents:
                logger.info(
                    f"✅ Property Documents Retrieved: {len(documents)} documents"
                )
    ...
    def test_comprehensive_property_info(self):
        ...
        try:
            comprehensive_data = self.api_client.get_comprehensive_property_info(
                self.test_apn
            )
            ...
            if comprehensive_data:
                ...
                self.failed_tests.append(
                    "Comprehensive Property Info: No data returned"
                )
                return False
    ...
    def test_error_handling(self):
        ...
        for invalid_apn in invalid_apns:
            try:
                ...
                if result is None:
                    logger.info(
                        f"   ✓ Correctly returned None for invalid APN: '{invalid_apn}'"
                    )
                else:
                    logger.warning(
                        f"   ⚠ Unexpected result for invalid APN '{invalid_apn}': {result}"
                    )
                    error_handling_passed = False

            except Exception as e:
                logger.info(
                    f"   ✓ Correctly handled exception for invalid APN '{invalid_apn}': {str(e)}"
                )
        ...
        if error_handling_passed:
            ...
            self.failed_tests.append(
                "Error Handling: Some invalid inputs produced unexpected results"
            )
    ...
    def generate_summary_report(self):
        ...
        pass_rate = (
            (len(self.passed_tests) / total_tests * 100) if total_tests > 0 else 0
        )
    ...
    def run_all_tests(self):
        ...
        for test_method in tests:
            try:
                ...
                logger.error(traceback.format_exc())
                self.failed_tests.append(
                    f"{test_method.__name__}: Crashed with {str(e)}"
                )
