    ...
    try:
        ...
        try:
            ...
            if not isinstance(sales_records, list):
                print(
                    f"   [ISSUE] Sales records should be list, got {type(sales_records)}"
                )
        ...
        try:
            ...
            print(
                f"   API tax history count: {len(api_tax_history) if isinstance(api_tax_history, list) else 'N/A'}"
            )
            ...
            if api_tax_history and isinstance(api_tax_history, list):
                ...
                for i, tax_record in enumerate(api_tax_history):
                    tax_data = {
                        ...
                        "assessed_value": (
                            int(tax_record.get("FullCashValue", 0))
                            if tax_record.get("FullCashValue")
                            else None
                        ),
                        "limited_value": (
                            int(tax_record.get("LimitedPropertyValue", "").strip())
                            if tax_record.get("LimitedPropertyValue", "").strip()
                            else None
                        ),
                    ...
                    if success:
                        print(
                            f"   [OK] Saved tax record {i+1}/{len(api_tax_history)} to database"
                        )
                    else:
        ...
        try:
            ...
            if comprehensive_data:
                ...
                success = db_manager.save_comprehensive_property_data(
                    comprehensive_data
                )
                if success:
        ...
        try:
            ...
            print(
                f"   Updated tax records: {len(updated_tax_records) if isinstance(updated_tax_records, list) else updated_tax_records}"
            )
            print(
                f"   Updated sales records: {len(updated_sales_records) if isinstance(updated_sales_records, list) else updated_sales_records}"
            )
