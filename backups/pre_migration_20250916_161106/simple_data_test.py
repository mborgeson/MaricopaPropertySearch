    ...
    def setup(self):
        ...
        try:
            ...
            if self.db_manager.test_connection():
                ...
                logger.warning(
                    "WARN: Database connection failed - some tests may be limited"
                )
        ...
        try:
            ...
            if property_data:
                ...
                if property_data.get("assessed_value"):
                    logger.info(
                        f"   Assessed Value: ${property_data.get('assessed_value'):,}"
                    )
                else:
    ...
    def test_error_handling(self):
        ...
        for invalid_apn in invalid_apns:
            try:
                ...
                if result is None:
                    logger.info(
                        f"   OK: Correctly returned None for invalid APN: '{invalid_apn}'"
                    )
                else:
                    logger.warning(
                        f"   WARN: Unexpected result for invalid APN '{invalid_apn}': {result}"
                    )
                    error_handling_passed = False

            except Exception as e:
                logger.info(
                    f"   OK: Correctly handled exception for invalid APN '{invalid_apn}': {str(e)}"
                )
