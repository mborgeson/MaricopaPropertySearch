...
def test_missouri_ave_search():
    ...
    try:
                ...
                if detailed_info:
                    ...
                    for field in key_fields:
                        ...
                        if value is not None:
                            if field == "latest_assessed_value" and isinstance(
                                value, (int, float)
                            ):
                                print(f"  {field}: ${value:,}")
                            else:
                    ...
                    if tax_history:
                        ...
                        for record in tax_history[:3]:  # Show first 3 records
                            ...
                            print(
                                f"  {year}: Full Cash Value ${value:,} | Limited Value ${limited:,}"
                                if isinstance(value, (int, float))
                                else f"  {year}: {value} | {limited}"
                            )
                    ...
                    try:
                        ...
                        print(
                            f"Property data insert: {'SUCCESS' if success else 'FAILED'}"
                        )
                    ...
                    try:
                        db_results = db_manager.search_properties_by_address(
                            "Missouri", limit=5
                        )
                        print(
                            f"Database address search: Found {len(db_results)} Missouri Ave properties"
                        )
                    except Exception as e:
        ...
        print(
            f"✓ Database: {'Connected' if db_manager.test_connection() else 'Issues'}"
        )
...
def test_data_collection_bottlenecks():
    ...
    try:
        ...
        for step_name, step_func in steps:
            ...
            print(
                f"{success} {step_name}: {avg_time:.2f}s avg (runs: {[f'{t:.2f}' for t in times]})"
            )
