...
def test_configuration():
    ...
    try:
        ...
        print_result(
            "API Configuration", True, f"Base URL: {api_config.get('base_url')}"
        )
        print_result(
            "API Token",
            bool(api_config.get("token")),
            f"Token: {'*'*10 + api_config.get('token', '')[-10:] if api_config.get('token') else 'None'}",
        )
...
def test_database_connection():
    ...
    try:
        ...
        if connection_ok:
            ...
            search_results = db_manager.search_properties_by_address(
                "Missouri", limit=10
            )
            print_result(
                "Address Search Query",
                True,
                f"Found {len(search_results)} Missouri Ave properties",
            )
        ...
        if search_results:
            ...
            if apn:
                ...
                print_result(
                    ...
                    detailed_info is not None,
                    f"Retrieved in {detail_time:.2f}s",
                )

                if detailed_info:
                    print(f"      Year Built: {detailed_info.get('year_built')}")
                    print(
                        f"      Living Area: {detailed_info.get('living_area_sqft')} sqft"
                    )
                    print(
                        f"      Latest Assessed Value: ${detailed_info.get('latest_assessed_value'):,}"
                    ...
                    print_result(
                        "Tax History",
                        True,
                        f"Found {len(tax_history)} tax records in {tax_time:.2f}s",
                    )
...
def test_web_scraping():
    ...
    try:
        ...
        print_result(
            "Owner Search",
            len(owner_results) >= 0,
            f"Found {len(owner_results)} properties in {search_time:.2f}s",
        )
        ...
        if owner_results:
            ...
            if apn:
                ...
                print_result(
                    "Property Scraping",
                    property_data is not None,
                    f"Scraped in {scrape_time:.2f}s",
                )

                if property_data:
                    print(f"      Scraped Owner: {property_data.get('owner_name')}")
                    print(
                        f"      Scraped Address: {property_data.get('property_address')}"
                    )
...
def test_background_data_collection():
    ...
    try:
        ...
        try:
            from src.improved_automatic_data_collector import (
                ImprovedMaricopaDataCollector,
            )

            collector = ImprovedMaricopaDataCollector(db_manager)
            print_result(
                "Data Collector Creation", True, "Collector initialized successfully"
            )
            ...
            try:
                ...
                print_result(
                    "Data Collection", True, f"Completed in {collection_time:.2f}s"
                )
                ...
                print_result(
                    "Data Collection",
                    False,
                    f"Error after {collection_time:.2f}s: {str(collection_error)[:100]}",
                )

        except ImportError as import_error:
            print_result(
                "Data Collector Import", False, f"Import error: {import_error}"
            )
...
def test_complete_search_workflow():
    ...
    try:
        ...
        print_result(
            "Initial Search",
            len(search_results) > 0,
            f"{len(search_results)} results in {search_time:.2f}s",
        )
        ...
        print_result(
            "Database Tax Data", True, f"Found {len(existing_tax)} tax records"
        )
        print_result(
            "Database Sales Data", True, f"Found {len(existing_sales)} sales records"
        )
        ...
        if detailed_info:
            ...
            try:
                ...
                print_result(
                    "Store Property Data", success, "Property data stored successfully"
                )
            except Exception as store_error:
                print_result(
                    "Store Property Data", False, f"Storage error: {store_error}"
                )
...
def generate_test_report(results):
    ...
    if not results.get("api"):
        print(
            "  * API client issues detected - check network connectivity and API token"
        )
