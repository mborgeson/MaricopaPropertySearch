...
def test_complete_property_with_tax_data():
    ...
    try:
        ...
        complete_info = client.get_complete_property_info_with_tax_scraping(
            test_apn, use_tax_scraping=True
        )
        ...
        if complete_info:
            ...
            if complete_info.get("current_tax_amount"):
                print(
                    f"✅ Current Tax Amount: ${complete_info['current_tax_amount']:,.2f}"
                )
            ...
            if (
                "tax_scrape_data" in complete_info
                and "tax_history" in complete_info["tax_scrape_data"]
            ):
                print(
                    f"✅ Tax History: {len(complete_info['tax_scrape_data']['tax_history'])} years available"
                )
