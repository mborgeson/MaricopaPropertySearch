...
def test_database_manager():
    ...
    try:
        ...
        print(
            f"[PASS] ConfigManager created, DB config: {db_config['host']}:{db_config['port']}"
        )
        ...
        if is_connected:
            ...
            if tax_insert:
                ...
                if len(tax_history) > 0:
                    print(
                        f"[PASS] Tax history retrieval successful ({len(tax_history)} records)"
                    )
            ...
            if sales_insert:
                ...
                if len(sales_history) > 0:
                    print(
                        f"[PASS] Sales history retrieval successful ({len(sales_history)} records)"
                    )
    ...
    test_categories = [
        ...
        (
            ...
            results.get("tax_insert", False) and results.get("tax_retrieve", False),
        ),
        (
            "Sales History Operations",
            results.get("sales_insert", False) and results.get("sales_retrieve", False),
        ),
    ...
    print(f"\nKEY FIXES VERIFICATION:")
    print("-" * 25)
    config_fix = results.get("config_manager", False) and results.get(
        "db_manager_init", False
    )
    ...
    print(
        f"[{'PASS' if apn_method_fix else 'FAIL'}] get_property_by_apn method working"
    )
