...
class TestSearchPerformance:
    ...
    def test_database_search_performance(
        self, test_database, performance_timer, app_config
    ):
        ...
        assert (
            elapsed < DATABASE_SEARCH_LIMIT
        ), f"Database search took {elapsed:.3f}s, limit is {DATABASE_SEARCH_LIMIT}s"
    ...
    def test_database_search_with_large_dataset(self, test_database, performance_timer):
        ...
        assert (
            elapsed < DATABASE_SEARCH_LIMIT * 1.5
        ), f"Large dataset search took {elapsed:.3f}s"
...
class TestUIResponsiveness:
    ...
    def test_search_type_change_responsiveness(
        self, qt_app, app_config, performance_timer
    ):
    ...
    def test_results_table_population_speed(
        self, qt_app, app_config, sample_search_results, performance_timer
    ):
...
class TestBackgroundProcessing:
    ...
    def test_background_enhancement_non_blocking(self, app_config, performance_timer):
        ...
        assert (
            elapsed < UI_RESPONSE_LIMIT
        ), f"Background thread start blocked for {elapsed:.3f}s"
    ...
    def test_api_enhancement_timeout(self, app_config, performance_timer):
        ...
        with patch.object(client, "search_by_owner") as mock_search:
            ...
            assert (
                elapsed <= API_ENHANCEMENT_LIMIT + 1
            ), f"API call took {elapsed:.3f}s, timeout should prevent this"
...
class TestMemoryUsage:
    ...
    def test_large_result_set_memory_usage(self, test_database):
        ...
        assert (
            memory_growth < 50
        ), f"Memory grew by {memory_growth:.2f}MB, should be < 50MB"
...
class TestApplicationStartupPerformance:
    ...
    def test_cold_startup_time(self, performance_timer):
        ...
        ), f"Application failed to start: {stderr.decode()}"
