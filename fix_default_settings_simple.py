...
def fix_apply_settings_method(content):
    ...
    old_method_start = 'def apply_settings_to_ui(self, settings_dict):'
    ...
    if start_idx == -1:
        print("Could not find apply_settings_to_ui method")
        return content
    
    end_idx = content.find(old_method_end, start_idx)
    if end_idx == -1:
        print("Could not find end of apply_settings_to_ui method")
        return content
...
def main():
    ...
    try:
        ...
        print()
        print("Fixed issues:")
        print("1. [FIXED] apply_settings_to_ui now properly sets UI checkbox states")
        print("2. [FIXED] Added support for auto-resizing table columns")
        print("3. [FIXED] Enhanced logging for better debugging")
        print()
...
if __name__ == "__main__":
    success = main()
    if success:
        print("\n[DONE] Settings fix applied successfully!")
        print("Restart the application to see the default settings applied.")
    else:
        print("\n[FAILED] Failed to apply settings fix.")