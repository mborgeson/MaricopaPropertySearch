...
class TaxSalesDataFlowDiagnostic:
    ...
    def check_existing_data(self, apn: str):
        ...
        try:
            ...
            if tax_count > 0:
                cursor.execute("SELECT tax_year, assessed_value, tax_amount FROM tax_history WHERE apn = %s ORDER BY tax_year DESC LIMIT 3", (apn,))
            ...
            if sales_count > 0:
                cursor.execute("SELECT sale_date, sale_price, seller_name FROM sales_history WHERE apn = %s ORDER BY sale_date DESC LIMIT 3", (apn,))
                sales_records = cursor.fetchall()
                print("   Recent sales records:")
                for record in sales_records:
                    print(f"     - {record['sale_date']}: ${record.get('sale_price', 'N/A')} from {record.get('seller_name', 'Unknown')}")
        ...
        try:
            ...
            if not missing_methods:
                ...
                try:
                    ...
                    print(f"   📊 get_tax_history returned {len(tax_result) if tax_result else 0} records")
                    ...
                    print("   ⏭️  Skipping sales history test to avoid long scraping delay")
        ...
        try:
            ...
            if not missing_methods:
                ...
                try:
                    ...
                    print(f"   📊 get_sales_history returned {len(sales_history)} records")

                    if len(tax_history) > 0 or len(sales_history) > 0:
                        self.test_results['sample_data_retrieval'] = True

                except Exception as method_error:
                    print(f"   ⚠️  Error calling database manager methods: {method_error}")
...
from src.enhanced_config_manager import EnhancedConfigManager
            ...
            print(f"   {'✅' if tax_inserted else '❌'} Sample tax record insertion: {'SUCCESS' if tax_inserted else 'FAILED'}")
            ...
            print(f"   {'✅' if sales_inserted else '❌'} Sample sales record insertion: {'SUCCESS' if sales_inserted else 'FAILED'}")
    ...
    def _cleanup_test_data(self, apn: str):
        ...
        try:
            ...
            # Remove test records
            cursor.execute("DELETE FROM tax_history WHERE apn = %s AND tax_year = 2024", (apn,))
            cursor.execute("DELETE FROM sales_history WHERE apn = %s AND recording_number = 'TEST123456'", (apn,))
...
def main():
    ...
    print("💡 Use this information to identify and fix the root cause of missing tax/sales history")
