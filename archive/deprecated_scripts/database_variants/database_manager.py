...
class DatabaseManager:
    ...
    def search_properties_by_owner(
        ...
        try:
            with self.get_connection() as conn:
                ...
                logger.debug(
                    f"DB_ANALYTICS: owner_search, query_time=measured, results={len(results)}, limit={limit}"
                )
    ...
    def search_properties_by_address(
        ...
        try:
            with self.get_connection() as conn:
                ...
                logger.debug(
                    f"DB_ANALYTICS: address_search, query_time=measured, results={len(results)}, limit={limit}"
                )
    ...
    def insert_tax_history(self, tax_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to insert tax history for {tax_data.get('apn', 'unknown')}: {e}"
            )
            return False
    ...
    def insert_sales_history(self, sales_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Failed to insert sales history for {sales_data.get('apn', 'unknown')}: {e}"
            )
            return False
    ...
    def get_database_stats(self) -> Dict[str, int]:
        ...
        try:
            with self.get_connection() as conn:
                ...
                # Recent searches
                cursor.execute(
                    "SELECT COUNT(*) FROM search_history WHERE searched_at > NOW() - INTERVAL '7 days'"
                )
