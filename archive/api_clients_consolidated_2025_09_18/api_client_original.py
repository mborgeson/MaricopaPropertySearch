...
class MaricopaAPIClient:
    ...
    def search_by_owner(self, owner_name: str, limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            if response and "Results" in response:
                ...
                logger.info(
                    f"SEARCH_ANALYTICS: owner_search, results={result_count}, limit={limit}"
                )
    ...
    def search_by_address(self, address: str, limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            if response and "Results" in response:
                ...
                # Log search analytics
                logger.info(
                    f"SEARCH_ANALYTICS: address_search, results={result_count}, limit={limit}"
                )
            ...
            if response and "Results" in response and len(response["Results"]) > 0:
                ...
                for result in response["Results"]:
                    if result.get("APN", "").replace("-", "").replace(
                        ...
                        logger.info(
                            f"SEARCH_ANALYTICS: apn_search, results=1, apn={apn}"
                        )
    ...
    def get_sales_history(self, apn: str, years: int = 10) -> List[Dict]:
        ...
        logger.warning(
            f"Sales history requires web scraping from Recorder's office - not available via API"
        )
    ...
    def get_property_documents(self, apn: str) -> List[Dict]:
        ...
        logger.warning(
            f"Property documents require web scraping from Recorder's office - not available via API"
        )
    ...
    def _validate_apn(self, apn: str) -> bool:
        ...
        return (
            len(clean_apn) >= 8
            and clean_apn.replace("-", "").replace(".", "").isdigit()
        )
        ...
        try:
            for endpoint, category in endpoints.items():
                ...
                if response and "Results" in response and response.get("TOTAL", 0) > 0:
                    ...
                    normalized_results = [
                        self._normalize_api_data(result)
            ...
            logger.info(
                f"Found {total_results} total results across all property types"
            )
    ...
    def get_detailed_property_data(self, apn: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Retrieved detailed data from {len(detailed_data)} endpoints for APN: {apn}"
        )
        ...
        try:
            ...
            if (
                ...
                comprehensive_info.update(
                    {
                        "year_built": self._safe_int(
                            res_details.get("ConstructionYear")
                        ),
                        "lot_size_sqft": self._safe_int(res_details.get("LotSize")),
                        "living_area_sqft": self._safe_int(
                            res_details.get("LivableSpace")
                        ),
                        ...
                        "garage_spaces": self._safe_int(
                            res_details.get("NumberOfGarages")
                        ),
                ...
                if "property_use_description" in comprehensive_info:
                    ...
                    if "Multiple Family" in use_desc:
                        comprehensive_info["land_use_code"] = (
            ...
            if "improvements" in detailed_data and detailed_data["improvements"]:
                ...
                if improvements:
                    ...
                    if residential_sqft > 0 and not comprehensive_info.get(
                        "living_area_sqft"
                    ):
    ...
    def get_complete_property_info_with_tax_scraping(
        self, apn: str, use_tax_scraping: bool = False
    ) -> Optional[Dict]:
        """
        ...
        """
        logger.info(
            f"Getting complete property info (with tax scraping: {use_tax_scraping}) for APN: {apn}"
        )
            ...
            if use_tax_scraping:
                try:
                    ...
                    if tax_data:
                        ...
                        if owner_info.get("owner_name") and not complete_info.get(
                            "owner_name"
                        ):
                            complete_info["owner_name"] = owner_info["owner_name"]
                        if owner_info.get("property_address") and not complete_info.get(
                            "property_address"
                        ):
                            complete_info["property_address"] = owner_info[
                                "property_address"
                            ]
                        if owner_info.get("mailing_address") and not complete_info.get(
                            "mailing_address"
                        ):
                            complete_info["mailing_address"] = owner_info[
                                "mailing_address"
                            ]
    ...
    def get_complete_property_with_automatic_data_collection(
        ...
        """
        logger.info(
            f"Getting complete property data with automatic collection for APN: {apn}"
        )
        ...
        try:
            ...
            logger.info(
                f"Existing data - Tax records: {len(existing_tax_records)}, Sales records: {len(existing_sales_records)}"
            )
            ...
            if needs_tax_data or needs_sales_data:
                logger.info(
                    f"APN {apn} needs data collection - Tax: {needs_tax_data}, Sales: {needs_sales_data}"
                )
                ...
                try:
                    ...
                    logger.info(
                        f"Data collection completed for APN {apn}: "
            ...
            logger.info(
                f"Complete property data compiled for APN {apn}: "
