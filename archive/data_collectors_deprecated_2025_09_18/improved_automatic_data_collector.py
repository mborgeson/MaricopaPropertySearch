    ...
    def collect_data_for_apn_sync(self, apn: str) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_scripts = sum(
                [api_success, tax_success, sales_success, recorder_success]
            )
            logger.info(
                f"Data collection completed for APN {apn}: {successful_scripts}/{total_scripts} scripts successful"
            )
    ...
    def _collect_api_data(self, apn: str, results: Dict) -> bool:
        ...
        try:
            ...
            if comprehensive_data:
                ...
                success = self.db_manager.save_comprehensive_property_data(
                    comprehensive_data
                )
    ...
    def _collect_tax_data(self, apn: str, results: Dict) -> bool:
        ...
        try:
            ...
            if tax_data and len(tax_data) > 0:
                ...
                logger.info(
                    f"Tax info script successful for APN {apn}: {len(tax_data)} records"
                )
                return True
            else:
    ...
    def _collect_sales_data(self, apn: str, results: Dict) -> bool:
        ...
        try:
            ...
            if existing_sales and len(existing_sales) > 0:
                ...
                logger.info(
                    f"Using existing sales data for APN {apn}: {len(existing_sales)} records"
                )
                return True
            else:
                results["errors"].append(
                    "Sales script ready but no existing data - browser automation needed"
                )
                return False
