    ...
    try:
        ...
        if search_results:
            ...
            if detailed_info:
                ...
                print(f"  Living Area: {detailed_info.get('living_area_sqft', 'N/A')} sqft")
                print(f"  Lot Size: {detailed_info.get('lot_size_sqft', 'N/A')} sqft")
                print(f"  Assessed Value: ${detailed_info.get('latest_assessed_value', 0):,}" if detailed_info.get('latest_assessed_value') else "  Assessed Value: N/A")
                print(f"  Property Use: {detailed_info.get('property_use_description', 'N/A')}")
                print(f"  Improvements: {detailed_info.get('improvements_count', 'N/A')}")
            ...
            if tax_history and len(tax_history) > 0:
                ...
                for i, record in enumerate(tax_history[:3]):
                    ...
                    print(f"  {year}: Full=${full_value:,} Limited=${limited_value:,}" if isinstance(full_value, (int, float)) else f"  {year}: {full_value} | {limited_value}")
            ...
            if detail_time > 5:
                ...
                print("This suggests API endpoint delays in get_comprehensive_property_info")
