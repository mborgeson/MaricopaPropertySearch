...
def test_specific_methods():
    ...
    try:
        ...
        if result:
            ...
            print(f"   - Payment Status: {result.get('scraped_data', {}).get('current_tax', {}).get('payment_status', 'N/A')}")
    ...
    try:
        ...
        if result and len(result) > 0:
            ...
            for i, sale in enumerate(result[:2]):  # Show first 2
                print(f"   - Sale {i+1}: {sale.get('sale_date', 'N/A')} - ${sale.get('sale_price', 'N/A'):,}" if sale.get('sale_price') else f"   - Sale {i+1}: {sale.get('sale_date', 'N/A')} - N/A")
        else:
    ...
    try:
        ...
        if result:
            ...
            print(f"   - Property data: {'Available' if result.get('property_address') or result.get('owner_name') else 'Limited'}")
    ...
    try:
        ...
        print(f"   ✓ SUCCESS: Invalid APN correctly raised exception: {type(e).__name__}")
    print()
