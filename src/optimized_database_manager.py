...
class OptimizedDatabaseManager(DatabaseManager):
    ...
    def _ensure_optimized_indexes(self, cursor):
        ...
        for index_sql in indexes:
            try:
                ...
                logger.debug(f"Index created/verified: {index_sql.split()[-1] if 'idx_' in index_sql else 'unknown'}")
            except Exception as e:
                logger.warning(f"Index creation skipped (may already exist): {e}")
    
    def advanced_property_search(self, 
                                search_term: str, 
        ...
        try:
            with self.get_connection() as conn:
                ...
                self._log_query_performance(search_type.value, query_time, len(results), total_count)
                
                return results, total_count
                
        except Exception as e:
            logger.error(f"Advanced search failed for {search_type.value} '{search_term}': {e}")
            return [], 0
    
    def _build_advanced_query(self, 
                             search_term: str, 
                             search_type: SearchType, 
                             filters: SearchFilters,
                             limit: int, 
                             offset: int) -> Tuple[str, str, List]:
    ...
    def get_property_suggestions(self, partial_term: str, search_type: SearchType, limit: int = 10) -> List[str]:
    ...
    def _log_query_performance(self, search_type: str, query_time: float, result_count: int, total_count: int):
        ...
        if query_time > 2.0:  # Queries taking more than 2 seconds
            logger.warning(f"Slow query detected - {search_type}: {query_time:.2f}s, {result_count}/{total_count} results")
        ...
        try:
            with self.get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    LIMIT 20
                """)
                stats['land_use_distribution'] = [dict(row) for row in cursor.fetchall()]
