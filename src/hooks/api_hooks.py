...
class APIRateLimitHook(Hook):
    ...
    async def execute(self, context: HookContext) -> HookResult:
        ...
        try:
            ...
            if self.blocked_until and current_time < self.blocked_until:
                ...
                logger.warning(f"API rate limit active - blocked for {wait_time:.1f}s more")
            ...
            if violations:
                ...
                logger.warning(f"API rate limit exceeded: {violations} - backing off for {backoff_time}s")
    ...
    def _check_rate_limits(self, current_time: float) -> List[str]:
        ...
        if len(recent_requests) > self.rate_limits['requests_per_second']:
            violations.append(f"requests_per_second: {len(recent_requests)}/{self.rate_limits['requests_per_second']}")
        ...
        if len(recent_requests) > self.rate_limits['requests_per_minute']:
            violations.append(f"requests_per_minute: {len(recent_requests)}/{self.rate_limits['requests_per_minute']}")

        # Check requests per hour
        recent_requests = [t for t in self.request_history if current_time - t <= 3600.0]
        if len(recent_requests) > self.rate_limits['requests_per_hour']:
            violations.append(f"requests_per_hour: {len(recent_requests)}/{self.rate_limits['requests_per_hour']}")
    ...
    def _get_rate_limit_info(self, current_time: float) -> Dict[str, int]:
        ...
        return {
            'requests_last_second': len([t for t in self.request_history if current_time - t <= 1.0]),
            'requests_last_minute': len([t for t in self.request_history if current_time - t <= 60.0]),
            'requests_last_hour': len([t for t in self.request_history if current_time - t <= 3600.0])
        }
...
class APIRequestResponseHook(Hook):
    ...
    async def _handle_request_start(self, context: HookContext, request_data: Dict) -> HookResult:
    ...
    async def _handle_response(self, context: HookContext, request_data: Dict) -> HookResult:
        """Handle API response"""
        ...
        if request_id not in self.active_requests:
            ...
            return HookResult(status=HookStatus.SUCCESS, result={'warning': 'Unknown request'})
        ...
        self.request_stats['status_codes'][status_key] = self.request_stats['status_codes'].get(status_key, 0) + 1
    ...
    async def _handle_request_error(self, context: HookContext, request_data: Dict) -> HookResult:
        ...
        if request_id in self.active_requests:
            ...
            logger.error(f"API request error: {req_info['method']} {req_info['url']} "
    ...
    def get_api_stats(self) -> Dict[str, Any]:
        ...
        success_rate = (self.request_stats['successful_requests'] / max(1, total_requests)) * 100
...
class APICacheHook(Hook):
    ...
    async def _check_cache(self, context: HookContext, request_data: Dict) -> HookResult:
    ...
    async def _store_cache(self, context: HookContext, request_data: Dict) -> HookResult:
        """Store API response in cache"""
        # Only cache successful GET requests
        if (request_data.get('method', 'GET').upper() != 'GET' or
            not (200 <= request_data.get('status_code', 0) < 300)):
...
def trigger_api_request(request_id: str, url: str, method: str = 'GET', params: Dict = None):
...
def trigger_api_response(request_id: str, status_code: int, response_data: Any,
