...
class PerformanceTrackingHook(Hook):
    ...
    async def _handle_operation_start(self, context: HookContext, perf_data: Dict) -> HookResult:
    ...
    async def _handle_operation_end(self, context: HookContext, perf_data: Dict) -> HookResult:
        ...
        if operation_id not in self.active_operations:
            ...
            return HookResult(status=HookStatus.SUCCESS, result={'warning': 'Unknown operation'})
        ...
        # Analyze performance
        performance_analysis = self._analyze_performance(op_info['name'], execution_time, end_system_metrics)
        ...
        return HookResult(
            ...
            metadata={'execution_time': execution_time, 'performance_level': performance_analysis['level']}
        )

    async def _handle_metric_update(self, context: HookContext, perf_data: Dict) -> HookResult:
        ...
        if not metric_name or metric_value is None:
            return HookResult(status=HookStatus.FAILED, result={'error': 'Missing metric name or value'})
        ...
        logger.debug(f"Performance metric updated: {metric_name} = {metric_value} {metric_unit}")
    ...
    def _analyze_performance(self, operation_name: str, execution_time: float, system_metrics: Dict) -> Dict[str, Any]:
        ...
        if execution_time > self.performance_thresholds['very_slow_operation']:
            ...
            analysis['issues'].append(f"Very slow execution time: {execution_time:.2f}s")
            analysis['suggestions'].append("Consider optimization or breaking into smaller operations")
        ...
        if 'process' in system_metrics:
            ...
            if memory_mb > self.performance_thresholds['memory_critical']:
                ...
                analysis['suggestions'].append("Investigate memory leaks or reduce memory usage")
        ...
        if len(self.operation_times[operation_name]) > 5:
            ...
            if execution_time > avg_time * 2:
                analysis['issues'].append(f"Execution time is {execution_time/avg_time:.1f}x slower than average")
                analysis['suggestions'].append("Compare with historical performance")

        return analysis

    def _log_performance(self, operation_name: str, execution_time: float, analysis: Dict):
        """Log performance based on analysis"""
        level = analysis['level']

        if level == 'very_poor':
            logger.error(f"VERY POOR PERFORMANCE: {operation_name} ({execution_time:.3f}s) - Issues: {analysis['issues']}")
        elif level == 'poor':
            logger.warning(f"POOR PERFORMANCE: {operation_name} ({execution_time:.3f}s) - Issues: {analysis['issues']}")
        elif level == 'warning':
            logger.warning(f"Performance warning: {operation_name} ({execution_time:.3f}s) - Issues: {analysis['issues']}")
        else:
        ...
        for operation_name, records in self.metrics_history.items():
            ...
            if recent_records:
                ...
                summary['operations'][operation_name] = {
                    ...
                    'slow_operations': len([t for t in execution_times if t > self.performance_thresholds['slow_operation']])
                }

                summary['performance_issues'] += summary['operations'][operation_name]['slow_operations']
...
class MemoryProfilerHook(Hook):
    ...
    async def execute(self, context: HookContext) -> HookResult:
        ...
        try:
            ...
            return HookResult(status=HookStatus.SUCCESS, result={'snapshot_taken': False})
    ...
    def _analyze_memory_leaks(self) -> Dict[str, Any]:
        ...
        potential_leak = (
            total_growth > self.memory_growth_threshold and
            avg_growth > 0 and
            sum(1 for g in growth_trend if g > 0) >= len(growth_trend) * 0.8  # 80% positive growth
        )
    ...
    def get_memory_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'time_range_hours': (snapshots[-1]['timestamp'] - snapshots[0]['timestamp']).total_seconds() / 3600,
            'current_memory_mb': memory_values[-1],
...
def track_metric(metric_name: str, metric_value: float, unit: str = '', metadata: Dict = None):
