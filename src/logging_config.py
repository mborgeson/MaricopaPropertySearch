...
class PerformanceLogger:
    ...
    def log_database_operation(
        self, operation: str, table: str, record_count: int = None
    ):
    ...
    def wrapper(*args, **kwargs):
                ...
                self.logger.info(
                    f"DB_START: {operation} on {table} [ID: {operation_id}]"
                )
...
class SearchLogger:
    ...
    def wrapper(*args, **kwargs):
                ...
                sanitized_term = (
                    search_term[:50] if len(search_term) > 50 else search_term
                )
...
class LoggingConfig:
    ...
    def _setup_file_handlers(self, formatter):
        ...
        main_log_file = self.log_dir / self.log_config.get(
            "file_name", "maricopa_property.log"
        )
    ...
    def _performance_filter(self, record):
        ...
        return any(
            ...
            for keyword in ["PERF_", "DB_", "API_", "SEARCH_"]
        )
...
_logging_config = None
    ...
    logger.error(
        f"Exception occurred{' in ' + context if context else ''}: {str(exception)}"
    )
    logger.error(f"Exception type: {type(exception).__name__}")
    logger.error(f"Traceback:\n{traceback.format_exc()}")
    def log_debug_variables(
    logger: logging.Logger, variables: Dict[str, Any], context: str = ""
):
