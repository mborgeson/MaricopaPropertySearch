...
class PerformanceTestSuite:
    ...
    def run_all_performance_tests(self) -> Dict[str, Any]:
        ...
        test_results["results"][
            "optimized_collector"
        ] = self.test_optimized_data_collector()
        ...
        test_results["performance_comparison"] = self.generate_performance_comparison(
            test_results["results"]
        )
    ...
    try:
            ...
            start_time = time.time()
            comprehensive_result = api_client.get_comprehensive_property_info(
                self.test_apn
            )
            ...
            return {
                ...
                "data_points_collected": (
                    len(comprehensive_result) if comprehensive_result else 0
                ),
            }

    except Exception as e:
            logger.error(f"Error testing original API client: {e}")
            return {"error": str(e), "success": False, "total_time": float("inf")}
    ...
    try:
            ...
            start_time = time.time()
            comprehensive_result = patch.get_comprehensive_property_info_fast(
                self.test_apn
            )
            comprehensive_time = time.time() - start_time
    ...
    try:
            ...
            start_time = time.time()
            comprehensive_result = api_client.get_comprehensive_property_info_fast(
                self.test_apn
            )
            comprehensive_time = time.time() - start_time
    ...
    try:
            collector = PerformanceOptimizedDataCollector(
    ...
    def progress_callback(results):
                ...
                logger.info(
                    f"Progressive update - Stage: {results.stage}, Time: {results.collection_time:.3f}s, Progress: {results.completion_percentage:.1f}%"
                )

            # Test progressive data collection
            start_time = time.time()
            results = collector.collect_property_data_sync(
                self.test_apn, callback=progress_callback
            )
    ...
    try:
            ...
            scrape_results = scraper.scrape_property_data_parallel(
                self.test_apn, ["tax", "sales", "documents"]
            )
            ...
            return {"error": str(e), "success": False, "scraping_time": float("inf")}
    finally:
    try:
    ...
    def generate_performance_comparison(
        ...
        for test_name, test_result in results.items():
            ...
            if (
                original_time
                and original_time != float("inf")
                and test_time != float("inf")
            ):
        ...
        best_performer = min(
            results.keys(), key=lambda x: results[x].get("total_time", float("inf"))
        )
        ...
        if best_performer:
            ...
            if best_time <= 3.0:
                comparison["recommendations"].append(
                    f"SUCCESS: {best_performer} achieves <3 second target ({best_time:.3f}s)"
                )
            else:
                comparison["recommendations"].append(
                    f"NEEDS IMPROVEMENT: Best performer {best_performer} still exceeds 3s target ({best_time:.3f}s)"
                )
    ...
    def print_performance_report(self, test_results: Dict[str, Any]):
        ...
        print(
                    f"  {test_name}: {improvement['improvement_percentage']:.1f}% faster ({improvement['speedup_factor']:.1f}x speedup)"
                )
...
from datetime import datetime
        ...
        results_file = (
            ...
            / f"performance_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
