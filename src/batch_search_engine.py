...
class BatchSearchRequest:
    ...
    def __eq__(self, other):
        if not isinstance(other, BatchSearchRequest):
            return False
        return self.identifier == other.identifier and self.search_type == other.search_type
    ...
    def acquire_scraper_token(self, timeout: float = 10.0) -> bool:
        ...
        while time.time() - start_time < timeout:
            with self.lock:
                ...
                self.scraper_bucket = min(self.burst_size, self.scraper_bucket + tokens_to_add)
...
class BatchSearchEngine:
    ...
    def __init__(self, 
        ...
        self.rate_limiter = RateLimiter(
            api_calls_per_second=2.0,
            scraper_calls_per_second=0.8
        )
    ...
    def submit_batch_search(self, 
        ...
        logger.info(f"Submitted batch job {job_id}: {len(requests)} requests "
    ...
    def _execute_batch_job(self, batch_job: BatchSearchJob):
        ...
        try:
            ...
            logger.info(f"Executing batch job {job_id} with {len(batch_job.requests)} requests")
            ...
            completed_count = sum(1 for req in batch_job.requests if req.completed_at is not None)
            successful_count = sum(1 for req in batch_job.requests if req.result is not None)
    ...
    def _execute_parallel(self, batch_job: BatchSearchJob):
        ...
        with ThreadPoolExecutor(max_workers=batch_job.max_concurrent) as executor:
            ...
            completed = 0
            for future in as_completed(future_to_request, timeout=batch_job.total_timeout):
                completed += 1
    ...
    def _execute_hybrid(self, batch_job: BatchSearchJob):
        ...
        high_priority = [r for r in sorted_requests if r.priority in [BatchPriority.URGENT, BatchPriority.HIGH]]
        normal_priority = [r for r in sorted_requests if r.priority == BatchPriority.NORMAL]
        ...
        if high_priority:
            ...
            completed += self._execute_batch_subset(high_priority, batch_job.max_concurrent)
    ...
    def _execute_batch_subset(self, requests: List[BatchSearchRequest], max_workers: int) -> int:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_request, timeout=300):  # 5 min timeout for subset
                completed_count += 1
                if self.shutdown_event.is_set():
                    break
    ...
    def _execute_single_request(self, request: BatchSearchRequest):
        ...
        logger.debug(f"Executing {request.search_type} search for: {request.identifier}")
        ...
        for attempt in range(request.max_retries + 1):
            try:
                ...
                logger.debug(f"Successfully completed {request.search_type} search for: {request.identifier}")
                return
                ...
                if attempt < request.max_retries:
                    ...
                    request.completed_at = datetime.now()
                    logger.error(f"Request failed after {request.max_retries + 1} attempts: {error_msg}")
    ...
    def get_job_status(self, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        with self.jobs_lock:
            ...
            completed_count = sum(1 for req in job.requests if req.completed_at is not None)
    ...
    def get_engine_statistics(self) -> Dict[str, Any]:
        ...
        with self.stats_lock:
            ...
            avg_processing_time = (self.total_processing_time / max(1, self.total_requests_processed))
            success_rate = (self.total_successful_requests / max(1, self.total_requests_processed)) * 100
    ...
    def run(self):
        ...
        try:
            ...
            if status and status['status'] == 'completed':
                ...
                self.job_completed.emit(self.current_job_id, {'results': results, 'status': status})
            else:
