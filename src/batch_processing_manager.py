...
class BatchProcessingManager:
    ...
    def __init__(
        ...
        if enable_background_collection:
            self.background_collector = BackgroundDataCollectionManager(
                db_manager=db_manager, max_concurrent_jobs=3
            )
        else:
        ...
        self.executor = ThreadPoolExecutor(
            max_workers=max_concurrent_jobs, thread_name_prefix="BatchProcessing"
        )
        ...
        logger.info(
            f"Batch processing manager initialized - "
    ...
    def submit_batch_job(
        ...
        logger.info(
            f"Submitted batch processing job {job_id}: "
    ...
    def _execute_batch_job(self, job: BatchProcessingJob, callback: Callable = None):
        ...
        try:
            ...
            with self.stats_lock:
                ...
                total_time = (
                    self.average_job_time * (self.total_jobs_processed - 1)
                    + execution_time
                )
                self.average_job_time = total_time / self.total_jobs_processed

            logger.info(
                f"Batch job {job.job_id} completed in {execution_time:.2f}s - "
    ...
    def _execute_property_search_job(
        ...
        if job.processing_mode == ProcessingMode.API_ONLY:
            ...
            completion_stats = self.batch_api_client.wait_for_batch_completion(
                request_ids, timeout=job.parameters.get("timeout", 300.0)
            )
            ...
            for request_id, result_data in completion_stats["results"].items():
                if result_data["status"]["success"]:
                    ...
                    job.failed_items += 1
                    job.errors.append(
                        f"API request {request_id}: {result_data['status'].get('error')}"
                    )
        ...
        elif job.processing_mode in [
            ProcessingMode.API_THEN_SCRAPING,
            ProcessingMode.INTELLIGENT,
        ]:
            # Use batch search engine for smart processing
            search_mode = (
                SearchMode.HYBRID
                if job.processing_mode == ProcessingMode.INTELLIGENT
                else SearchMode.SEQUENTIAL
            )
            ...
            batch_job_id = self.batch_search_engine.submit_batch_search(
                ...
                callback=lambda batch_job: self._update_job_progress(
                    job, batch_job, callback
                ),
            )
                ...
                if results:
                    for result in results:
                        if result["success"]:
                            ...
                            job.failed_items += 1
                            job.errors.append(
                                f"Search {result['identifier']}: {result['error']}"
                            )
    ...
    def _execute_data_enhancement_job(
        ...
        enhancement_result = self.background_collector.collect_batch_data(
            job.identifiers, priority=job.priority
        )

        if not enhancement_result["success"]:
            raise ValueError(
                f"Failed to start batch enhancement: {enhancement_result['error']}"
            )
        ...
        while True:
            ...
            if status.get("total_jobs", 0) > 0:
                job.progress = (
                    status.get("completed_jobs", 0) / status["total_jobs"]
                ) * 100
                job.completed_items = status.get("completed_jobs", 0)
                job.successful_items = status.get(
                    "completed_jobs", 0
                )  # Simplified for now
            ...
            # Check if done
            if status.get("pending_jobs", 1) == 0 and status.get("active_jobs", 1) == 0:
                break
    ...
    def _execute_comprehensive_collection_job(
        ...
        logger.info(
            f"Executing comprehensive collection for {len(job.identifiers)} properties"
        )

        # Phase 1: API Collection
        logger.info("Phase 1: API data collection")
        api_request_ids = self.batch_api_client.batch_search_by_apns(
            job.identifiers, priority=job.priority.value, comprehensive=True
        )

        api_stats = self.batch_api_client.wait_for_batch_completion(
            api_request_ids, timeout=job.parameters.get("api_timeout", 180.0)
        )
        ...
        if self.parallel_scraper and job.parameters.get("enable_scraping", True):
            ...
            if scraping_requests:
                scraping_request_ids = self.parallel_scraper.submit_scraping_requests(
                    scraping_requests
                )
                ...
                while len(completed_scraping) < len(scraping_request_ids):
                    for request_id in scraping_request_ids:
                        ...
                        if status and status["status"] == "completed":
                            ...
                            if status["success"]:
                                result = self.parallel_scraper.get_request_result(
                                    request_id
                                )
                                job.results[request_id] = result
                            else:
                                job.errors.append(
                                    f"Scraping {request_id}: {status['error']}"
                                )

                    # Update progress
                    total_operations = len(api_request_ids) + len(scraping_request_ids)
                    completed_operations = len(api_stats["results"]) + len(
                        completed_scraping
                    )
    ...
    def _execute_tax_collection_job(
        ...
        logger.info(
            f"Executing tax data collection for {len(job.identifiers)} properties"
        )
        ...
        request_ids = self.parallel_scraper.batch_scrape_tax_data(
            job.identifiers, priority=job.priority.value
        )
            ...
            for request_id in request_ids:
                ...
                if status and status["status"] == "completed":
                    ...
                    if status["success"]:
                        ...
                        job.errors.append(
                            f"Tax scraping {request_id}: {status['error']}"
                        )
                        job.failed_items += 1
    ...
    def _execute_sales_collection_job(
        ...
        logger.info(
            f"Executing sales data collection for {len(job.identifiers)} properties"
        )
            ...
            for request_id in request_ids:
                ...
                if status and status["status"] == "completed":
                    ...
                    if status["success"]:
                        ...
                        job.errors.append(
                            f"Sales scraping {request_id}: {status['error']}"
                        )
                        job.failed_items += 1
    ...
    def _execute_bulk_validation_job(
        ...
        if validation_requests:
            request_ids = self.batch_api_client.submit_batch_requests(
                validation_requests
            )

            completion_stats = self.batch_api_client.wait_for_batch_completion(
                request_ids, timeout=job.parameters.get("timeout", 120.0)
            )
            ...
            for request_id, result_data in completion_stats["results"].items():
                if result_data["status"]["success"] and result_data["result"]:
                    ...
                    job.results[request_id] = {
                        "valid": False,
                        "error": result_data["status"].get("error"),
                    }
            ...
            if job_id in self.completed_jobs:
                ...
                return {
                    ...
                    "statistics": {
                        ...
                        "successful_items": job.successful_items,
                        "failed_items": job.failed_items,
                        "success_rate": (job.successful_items / max(1, job.total_items))
                        * 100,
                    },
                }
                ...
                return {
                    ...
                    "partial_results": self.active_jobs[job_id].results,
                }
            else:
                return None
    ...
    def get_manager_statistics(self) -> Dict[str, Any]:
        ...
        with self.stats_lock:
            success_rate = (
                self.total_successful_items / max(1, self.total_items_processed)
            ) * 100
        ...
        stats = {
            ...
            "components": {
                ...
                "parallel_scraper": (
                    self.parallel_scraper.get_scraper_statistics()
                    if self.parallel_scraper
                    else None
                ),
                "background_collector": (
                    self.background_collector.get_collection_status()
    ...
    def cleanup_completed_jobs(self, max_age_hours: int = 24):
        ...
        self.batch_api_client.cleanup_completed_requests(
            max_age_hours // 4
        )  # More frequent for API requests
        ...
        try:
            ...
            if status:
                if status["status"] == "completed":
                    results = self.processing_manager.get_job_results(
                        self.current_job_id
                    )
