...
class SearchCache:
    ...
    def _generate_key(self, search_type: str, search_term: str, filters: Dict = None) -> str:
    ...
    def put(self, search_type: str, search_term: str, results: List[Dict], 
...
class SearchHistory:
    ...
    def add_search(self, search_type: str, search_term: str, results_count: int) -> None:
    ...
    def get_suggestions(self, partial_term: str, search_type: str = None, limit: int = 10) -> List[str]:
        ...
        with self.lock:
            ...
            matching_terms = [
                (term, frequency) for term, frequency in self.term_frequency.items()
                if partial_lower in term and len(term) > len(partial_lower)
            ]
            
            # Sort by frequency and recency
            matching_terms.sort(key=lambda x: (-x[1], -time.time()))  # Frequency desc, then recency
            ...
            if search_type:
                ...
                for term, _ in matching_terms:
                    for entry in reversed(self.history):
                        if entry['term'].lower() == term and entry['type'] == search_type:
    ...
    def get_popular_terms(self, search_type: str = None, limit: int = 10) -> List[Tuple[str, int]]:
        ...
        with self.lock:
            if search_type:
                ...
                sorted_terms = sorted(type_terms.items(), key=lambda x: x[1], reverse=True)
                return sorted_terms[:limit]
            else:
                sorted_terms = sorted(self.term_frequency.items(), key=lambda x: x[1], reverse=True)
                return sorted_terms[:limit]