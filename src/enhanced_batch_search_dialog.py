...
class EnhancedBatchSearchDialog(QDialog):
    ...
    def setup_input_tab(self, tab):
        ...
        self.job_type.addItem(
            "Comprehensive Search", BatchSearchJobType.COMPREHENSIVE_SEARCH
        )
    ...
    def setup_options_tab(self, tab):
        ...
        self.max_concurrent.setToolTip(
            "Number of parallel searches (1-10). Higher values = faster but more resource intensive."
        )
        ...
        self.enable_background_collection = QCheckBox(
            "Enable background data collection after search"
        )
        self.enable_background_collection.setChecked(True)
        self.enable_background_collection.setToolTip(
            "Automatically enhance search results with additional data"
        )
        ...
        self.comprehensive_data.setToolTip(
            "Include tax records, sales history, and detailed property information"
        )
        ...
        self.enable_web_scraping.setToolTip(
            "Use web scraping to gather additional property details (slower)"
        )
        ...
        # Error handling
        self.continue_on_errors = QCheckBox(
            "Continue processing if individual searches fail"
        )
        ...
        for line in lines:
            if search_type == "apn":
                ...
                if len(line) > 10 and any(
                    state in line.upper() for state in ["AZ", "ARIZONA"]
                ):
                    valid_count += 1
                else:
                    invalid_count += 1
    ...
    def import_from_file(self, file_type):
        ...
        if file_type == "txt":
            filename, _ = QFileDialog.getOpenFileName(
                self, "Import Text File", "", "Text Files (*.txt);;All Files (*)"
            )
        else:  # CSV
            filename, _ = QFileDialog.getOpenFileName(
    ...
    def start_batch_search(self):
        ...
        try:
            ...
            self.status_label.setText(
                f"Starting batch {job_type.value} for {len(search_terms)} items..."
            )
    ...
    def _update_progress_display(self):
        ...
        try:
            ...
            if status:
                ...
                self.stats_labels["processed"].setText(
    ...
    def _handle_batch_completion(self):
        ...
        try:
            ...
            if self.batch_results:
                ...
                logger.info(
                    f"Batch search {self.current_job_id} completed successfully"
                )
            else:
