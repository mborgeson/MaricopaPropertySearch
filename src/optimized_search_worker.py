...
class OptimizedSearchWorker(QThread):
    ...
    def __init__(
        ...
        self.use_cache = (
            use_cache and not fresh_data_only
        )  # Disable cache if fresh data only
    ...
    def run(self):
        ...
        try:
            ...
            if self.use_cache and not self.fresh_data_only:
                ...
                if cached_results is not None:
                    ...
                    self.status_updated.emit(
                        f"Found {len(cached_results)} properties (cached)"
                    )
                    ...
                    self.history.add_search(
                        search_type.value, sanitized_term, len(filtered_results)
                    )
                    return
            elif self.fresh_data_only:
                self.status_updated.emit("Fresh data only mode - skipping cache...")
                logger.info(
                    f"Fresh data only mode enabled for {search_type.value} search: {sanitized_term}"
                )
            ...
            if self.fresh_data_only:
                ...
                db_results = self._search_external_sources_fresh(
                    sanitized_term, search_type
                )
                total_count = len(db_results)
            else:
                # Perform database search
                db_results, total_count = self._search_database(
                    sanitized_term, search_type
                )
            ...
            if (
                not db_results
                and search_type != SearchType.APN
                and not self.fresh_data_only
            ):
                ...
                logger.warning(
                    f"Fresh data collection returned no results for {search_type.value}: {sanitized_term}"
                )
            ...
            if self.use_cache and db_results and not self.fresh_data_only:
                ...
                logger.info(
                    f"Fresh data only mode - not caching {len(db_results)} results"
                )
            ...
            self.status_updated.emit(
                f"Found {total_count} properties in {search_duration:.2f}s{data_source_msg}"
            )
            self.results_ready.emit(db_results, total_count)
            self.search_completed.emit(
                search_type.value, sanitized_term, total_count, search_duration
            )

            logger.info(
                f"Search completed: {search_type.value} '{sanitized_term}' -> {total_count} results in {search_duration:.2f}s"
            )
    ...
    def _search_database(
        ...
        try:
            ...
            logger.debug(
                f"Database search returned {len(results)}/{total_count} results"
            )
    ...
    def _search_external_sources(
        ...
        try:
            ...
            if not external_results and search_type in [
                SearchType.OWNER,
                SearchType.APN,
            ]:
    ...
    def _search_external_sources_fresh(
        ...
        try:
            ...
            if search_type == SearchType.OWNER:
                ...
                api_result = self.api_client.get_comprehensive_property_info(
                    search_term
                )
            ...
            if not fresh_results and search_type in [SearchType.OWNER, SearchType.APN]:
                ...
                logger.info(
                    f"Fresh web scraping returned {len(scrape_results)} results"
                )
    ...
    def _cache_results(
        ...
        try:
            ...
            logger.debug(
                f"Cached {len(results)} results for {search_type.value}: '{search_term}'"
            )
        except Exception as e:
