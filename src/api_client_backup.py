...
class MaricopaAPIClient:
    ...
    def search_by_owner(self, owner_name: str, limit: int = 50) -> List[Dict]:
        ...
        while len(results) < limit:
            ...
            if len(results) >= total_found or len(properties) < 25:  # API returns 25 per page
                break
    ...
    def get_property_details(self, apn: str) -> Optional[Dict]:
        ...
        residential_data = self._make_request(f'/parcel/{clean_apn}/residential-details')
        if residential_data:
    ...
    def get_tax_history(self, apn: str, years: int = 5) -> List[Dict]:
        ...
        if response:
            ...
            logger.info(f"Retrieved {len(tax_history)} valuation records for APN: {apn}")
            return tax_history
        else:
    ...
    def get_property_documents(self, apn: str) -> List[Dict]:
        ...
        if response and 'documents' in response:
            logger.info(f"Retrieved {len(response['documents'])} documents for APN: {apn}")
    ...
    def _format_address(self, raw_data: Dict) -> str:
        ...
        return ' '.join(address_parts) if address_parts else raw_data.get('PropertyAddress', '')
    ...
    def _extract_address_info(self, address_data: Dict) -> Dict:
        ...
        return {
            'property_address': self._format_address(address_data)
        }
...
class MockMaricopaAPIClient(MaricopaAPIClient):
    ...
    def search_by_owner(self, owner_name: str, limit: int = 50) -> List[Dict]:
        ...
        return [self._generate_mock_property(f"12345{i:03d}") for i in range(min(3, limit))]
    
    def search_by_address(self, address: str, limit: int = 50) -> List[Dict]:
        """Mock search by address"""
        logger.info(f"Mock: Searching properties by address: {address}")
        return [self._generate_mock_property(f"67890{i:03d}") for i in range(min(2, limit))]
