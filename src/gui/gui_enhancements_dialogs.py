...
class ApplicationSettingsDialog(QDialog):
    ...
    def load_settings(self):
        try:
            ...
            self.auto_start_collection.setChecked(self.config.get('auto_start_collection', True))
            self.default_search_type.setCurrentText(self.config.get('default_search_type', 'Property Address'))
            ...
            self.auto_resize_columns.setChecked(self.config.get('auto_resize_columns', True))
            self.status_update_freq.setValue(self.config.get('status_update_frequency', 2))
            self.show_progress_details.setChecked(self.config.get('show_progress_details', True))
            self.always_fresh_data.setChecked(self.config.get('always_fresh_data', True))
...
class DataCollectionSettingsDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.priority_balanced = QRadioButton("Balanced - Equal priority for all sources")
        self.priority_speed = QRadioButton("Speed - Prioritize fastest sources")
        self.priority_comprehensive = QRadioButton("Comprehensive - Prioritize data completeness")
...
class CacheManagementDialog(QDialog):
    ...
    def clear_all_cache(self):
        reply = QMessageBox.question(self, "Clear All Cache",
        ...
        if reply == QMessageBox.Yes:
            try:
                if self.background_manager and self.background_manager.worker:
                    ...
                    QMessageBox.information(self, "Success", "All cache cleared successfully.")
                    self.refresh_cache_stats()
    ...
    def clear_old_entries(self):
        ...
        reply = QMessageBox.question(self, "Clear Old Entries",
        ...
        if reply == QMessageBox.Yes:
            try:
                if self.background_manager and self.background_manager.worker:
                    ...
                    self.background_manager.worker.cache.clear_old_entries(max_age_seconds)
                    QMessageBox.information(self, "Success", f"Old entries cleared successfully.")
                    self.refresh_cache_stats()
                    
            except Exception as e:
                QMessageBox.critical(self, "Error", f"Error clearing old entries: {str(e)}")
...
class BatchSearchDialog(QDialog):
    ...
    def on_batch_completed(self, results):
        ...
        if self.enable_collection.isChecked() and self.background_manager:
            ...
            if jobs_queued > 0:
                QMessageBox.information(self, "Batch Search Complete",
...
class BatchSearchWorker(QThread):
    ...
    def __init__(self, search_terms, search_type, db_manager, api_client, max_concurrent):
        super().__init__()
...
class ParallelProcessingDialog(QDialog):
    ...
    def apply_settings(self):
        ...
        QMessageBox.information(self, "Settings Applied", "Parallel processing settings have been applied.")
