...
class EnhancedSearchWorker(QThread):
    ...
    try:
            self.status_updated.emit(
                f"Searching {self.search_type}: {self.search_term}..."
            )
            ...
            if results and self.background_manager.is_running():
                jobs_queued = self.background_manager.enhance_search_results(
                    results, max_properties=25
                )
                if jobs_queued > 0:
                    self.background_collection_started.emit(jobs_queued)
                    logger.info(
                        f"Queued {jobs_queued} properties for prioritized background data enhancement"
                    )
                    # Emit status update immediately
                    self.status_updated.emit(
                        f"Auto-collecting data for {jobs_queued} properties..."
                    )
                else:
                    logger.info(
                        "No new properties queued (already have recent data or in progress)"
                    )
                    self.status_updated.emit(
                        "All properties have recent data or are being processed"
                    )
    ...
    def _search_by_owner(self) -> List[Dict]:
        ...
        if not db_results:
            ...
            comprehensive_results = self.api_client.search_all_property_types(
                self.search_term
            )
...
class BatchCollectionTracker(QObject):
    ...
    def _complete_batch(self):
        ...
        failed = sum(1 for r in self.batch_results.values() if r.get("error"))
        skipped = sum(1 for r in self.batch_results.values() if r.get("skipped"))
        ...
        logger.info(
            f"Batch collection completed: {successful} successful, "
...
class BackgroundCollectionStatusWidget(QWidget):
    ...
    def _on_batch_started(self, total_properties):
        ...
        self.batch_status_label.setText(
            f"Collecting data for {total_properties} properties..."
        )
    ...
    def _on_batch_progress(self, completed, total):
        ...
        self.batch_status_label.setText(
            f"Collecting... {completed}/{total} complete ({percentage:.0f}%)"
        )
...
class PropertyDetailsDialog(QDialog):
    ...
    def __init__(
        ...
        background_manager: BackgroundDataCollectionManager,
        parent=None,
    ):
        super().__init__(parent)
    ...
    def setup_ui(self):
        """Setup dialog UI"""
        self.setWindowTitle(
            f"Property Details - {self.property_data.get('apn', 'Unknown')}"
        )
        self.setModal(True)
        self.resize(900, 700)
    ...
    def load_property_details(self):
        ...
        if tax_history:
            ...
            self.tax_status_label.setText(
                "Tax data: No records found - will auto-collect in background"
            )
        ...
        if sales_history:
            self.sales_status_label.setText(
                f"Sales data: {len(sales_history)} records found"
            )
            self.sales_status_label.setStyleSheet("color: green;")
        else:
            self.sales_status_label.setText(
                "Sales data: No records found - will auto-collect in background"
            )
    ...
    def manual_collect_data(self):
        ...
        progress = QProgressDialog(
            "Collecting property data...", "Cancel", 0, 100, self
        )
    ...
    try:
            ...
            if result and (
                result.get("tax_data_collected") or result.get("sales_data_collected")
            ):
                ...
                QMessageBox.warning(
                    self, "Warning", f"Could not collect complete data for APN {apn}"
                )
            ...
            if success:
                ...
                if hasattr(self, "status_widget"):
                    self.status_widget.show_message(
                        "Automatically collecting comprehensive property data...",
                        "info",
                    )
            else:
                logger.info(
                    f"APN {apn} already has recent data or collection in progress"
                )
        else:
    ...
    try:
            ...
            needs_collection = not (
                has_recent_tax and has_recent_sales and has_property_details
            )
    ...
    def _on_background_job_completed(self, apn: str, result: Dict):
        ...
        if apn == self.property_data.get("apn"):
            logger.info(
                f"Background collection completed for APN {apn}, refreshing dialog"
            )
            self.collection_in_progress = False
            ...
            if hasattr(self, "tax_status_label"):
                self.tax_status_label.setText(
                    f"Tax data: {tax_count} records collected ✓"
                )
                self.tax_status_label.setStyleSheet("color: green; font-weight: bold;")

            if hasattr(self, "sales_status_label"):
                self.sales_status_label.setText(
                    f"Sales data: {sales_count} records collected ✓"
                )
                self.sales_status_label.setStyleSheet(
            ...
            QTimer.singleShot(
                3000, lambda: self.setWindowTitle(f"Property Details - {apn}")
            )
    ...
    def _update_collection_progress_display(self):
        ...
        if self.collection_in_progress:
            ...
            if hasattr(self, "sales_status_label"):
                self.sales_status_label.setText(
                    "Sales data: Collection in progress... 🔄"
                )
                self.sales_status_label.setStyleSheet(
    ...
    def refresh_property_data(self):
        ...
        progress = QProgressDialog(
            "Refreshing property data...", "Cancel", 0, 100, self
        )
        progress.setWindowModality(Qt.WindowModal)
        progress.show()
        progress.setValue(30)
    ...
    try:
            ...
            if self.background_manager and self.background_manager.is_running():
                ...
                success = self.background_manager.collect_data_for_apn(
                    apn, JobPriority.CRITICAL, force_refresh=True
                )

                if success:
                    progress.setValue(90)
                    progress.setLabelText(
                        "Collection queued - dialog will refresh automatically..."
                    )
                    QMessageBox.information(
                        self,
                        "Refresh Started",
            ...
            QMessageBox.critical(
                self, "Refresh Error", f"Error refreshing property data: {str(e)}"
            )
    ...
    def populate_tax_table(self, tax_history: List[Dict]):
        ...
        for i, record in enumerate(tax_history):
            self.tax_table.setItem(
                i, 0, QTableWidgetItem(str(record.get("tax_year", "")))
            )

            # Handle assessed_value
            assessed_value = record.get("assessed_value")
            assessed_text = (
                f"${assessed_value:,.2f}" if assessed_value is not None else "N/A"
            )
            ...
            limited_text = (
                f"${limited_value:,.2f}" if limited_value is not None else "N/A"
            )
            ...
            self.tax_table.setItem(
                i, 4, QTableWidgetItem(record.get("payment_status", "") or "N/A")
            )
    ...
    def populate_sales_table(self, sales_history: List[Dict]):
        ...
        for i, record in enumerate(sales_history):
            self.sales_table.setItem(
                i, 0, QTableWidgetItem(str(record.get("sale_date", "")))
            )
            ...
            self.sales_table.setItem(
                i, 2, QTableWidgetItem(record.get("seller_name", ""))
            )
            self.sales_table.setItem(
                i, 3, QTableWidgetItem(record.get("buyer_name", ""))
            )
            self.sales_table.setItem(
                i, 4, QTableWidgetItem(record.get("deed_type", ""))
            )
            self.sales_table.setItem(
                i, 5, QTableWidgetItem(record.get("recording_number", ""))
            )
...
class EnhancedPropertySearchApp(QMainWindow):
    ...
    try:
                ...
                logger.warning(
                    f"Failed to initialize real API client: {e}. Using mock client."
                )
    ...
    try:
                ...
                logger.warning(
                    f"Failed to initialize real web scraper: {e}. Using mock scraper."
                )
    ...
    try:
            ...
            self.status_bar.showMessage(
                "Background collection failed to start - check logs"
            )
    def setup_ui(self):
        ...
        self.bg_status_widget.start_stop_btn.clicked.connect(
            self.toggle_background_collection
        )
        ...
        self.collect_all_btn.setToolTip(
            "Collect complete property data for ALL visible search results"
        )
    ...
    def setup_connections(self):
        ...
        self.background_manager.collection_started.connect(
            self._setup_worker_connections
        )

        # Connect batch completion to button reset
        self.bg_status_widget.batch_tracker.batch_completed.connect(
            self._on_batch_collection_completed
        )
    def _setup_worker_connections(self):
        ...
        if self.background_manager.worker:
            self.background_manager.worker.job_started.connect(
                self.on_background_job_started
            )
            self.background_manager.worker.job_failed.connect(
                self.on_background_job_failed
            )
    ...
    def setup_menu_bar(self):
        ...
        collection_settings_action.triggered.connect(
            self.show_collection_settings_dialog
        )
        ...
        start_collection_action.triggered.connect(
            lambda: self.background_manager.start_collection()
        )
        collection_menu.addAction(start_collection_action)

        stop_collection_action = QAction("Stop Background Collection", self)
        stop_collection_action.triggered.connect(
            lambda: self.background_manager.stop_collection()
        )
    ...
    def perform_search(self):
        ...
        self.search_worker.background_collection_started.connect(
            self.on_background_collection_started
        )
...
import json
            ...
            owner_name = (
                ...
                or raw_data.get("Ownership")
                or "Owner Info Available"
            )
            self.results_table.setItem(i, 1, QTableWidgetItem(str(owner_name)))

            # Property Address
            self.results_table.setItem(
                i, 2, QTableWidgetItem(result.get("property_address", ""))
            )

            # Year Built
            year_built = (
                result.get("year_built")
                or raw_data.get("YearBuilt")
                or raw_data.get("Year Built")
            )
            ...
            # Lot Size
            lot_size = result.get("lot_size_sqft") or raw_data.get("LotSize")
    ...
    try:
            ...
            collecting = apn in [
                job.get("apn", "") for job in bg_status.get("active_jobs", [])
            ]
    ...
    try:
                ...
                logger.debug(
                    f"Data types for {apn}: tax_records={tax_type}, sales_records={sales_type}"
                )
    except:
    ...
    def on_background_collection_started(self, jobs_queued: int):
        ...
        self.status_bar.showMessage(
            f"Auto-collection: {jobs_queued} properties queued for data enhancement",
            3000,
        )
        ...
        logger.info(
            f"Auto-collection started for {jobs_queued} properties from search results"
        )
    ...
    def on_background_job_completed(self, apn: str, result: Dict):
        ...
        data_status = self._get_data_collection_status(apn)
        color = (
            QColor(200, 255, 200) if data_status["complete"] else QColor(255, 255, 200)
        )
        ...
        self.status_bar.showMessage(
            ...
            f"Tax: {tax_count}, Sales: {sales_count}",
            5000,
        )

        logger.info(f"Updated table display for completed collection: APN {apn}")
    def _update_table_status_for_apn(
        self, apn: str, status_text: str, background_color: QColor
    ):
    ...
    def collect_all_data(self):
        ...
        if not self.current_results:
            QMessageBox.warning(
                self, "Warning", "No search results to collect data for."
            )
            return
    ...
    def _do_collect_all_data(self):
        ...
        all_apns = [
            result.get("apn") for result in self.current_results if result.get("apn")
        ]

        if not all_apns:
            QMessageBox.warning(
                self, "Warning", "No valid APNs found in search results."
            )
            return
    ...
    try:
        ...
        batch_result = self.background_manager.collect_batch_data(
            apns_to_collect, JobPriority.HIGH
        )
        ...
        if jobs_added > 0:
            # Update status bar
            self.status_bar.showMessage(
                f"Batch collection started: {jobs_added} properties queued"
            )
            ...
            QMessageBox.warning(
                self, "Warning", "No properties were queued for collection."
            )
    ...
    def _on_batch_collection_completed(self, summary: Dict):
        ...
        # Update status bar
        self.status_bar.showMessage(
            f"Batch collection completed: {successful} successful, {failed} failed",
            10000,
        )
    ...
    def view_property_details(self):
        ...
        if row_index < len(self.current_results):
            ...
            dialog = PropertyDetailsDialog(
                property_data, self.db_manager, self.background_manager, self
            )
            dialog.exec_()
                ...
                with open(filename, "w", newline="", encoding="utf-8") as csvfile:
                    if self.current_results:
                        writer = csv.DictWriter(
                            csvfile, fieldnames=self.current_results[0].keys()
                        )
                        writer.writeheader()
    ...
    def refresh_current_data(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            if apns:
                ...
                batch_result = self.background_manager.collect_batch_data(
                    apns, JobPriority.NORMAL
                )
                jobs_added = batch_result.get("jobs_added", 0)

                if jobs_added > 0:
                    self.status_bar.showMessage(
                        f"Refresh queued: {jobs_added} properties", 5000
                    )
                    QMessageBox.information(
                        self,
                        "Refresh Started",
    ...
    def force_data_collection(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            if self.background_manager and self.background_manager.worker:
                ...
                batch_result = self.background_manager.collect_batch_data(
                    apns, JobPriority.CRITICAL
                )
                jobs_added = batch_result.get("jobs_added", 0)

                if jobs_added > 0:
                    self.status_bar.showMessage(
                        f"Force collection started: {jobs_added} properties", 5000
                    )
                    QMessageBox.information(
                        self,
                        "Force Collection Started",
                    ...
                    QMessageBox.warning(
                        self,
                        "Force Collection",
                        "No properties could be queued for collection.",
                    )
    ...
    try:
                if self.background_manager and self.background_manager.worker:
                    ...
                    QMessageBox.warning(
                        self,
                        "Cache Clear",
                        "Background worker not available to clear cache.",
                    )
    except Exception as e:
                logger.error(f"Error clearing cache: {e}")
                QMessageBox.critical(
                    self, "Cache Clear Error", f"Error clearing cache: {str(e)}"
                )
    ...
    def show_data_source_dialog(self):
        ...
        dialog = DataSourceConfigurationDialog(
            self.config, self.api_client, self.scraper, self
        )
        dialog.exec_()
    def show_about(self):
        """Show about dialog"""
    ...
    try:
            ...
            if self.background_manager.is_running():
                logger.info(
                    "Stopping background collection for application shutdown..."
                )
                self.background_manager.stop_collection()
