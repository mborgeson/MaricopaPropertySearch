...
class BatchSearchIntegrationManager:
    ...
    def execute_batch_search(
        ...
        logger.info(
            f"Starting batch search job {job_id}: "
    ...
    def _execute_basic_search(
        ...
        engine_job_id = self.batch_search_engine.submit_batch_search(
            ...
            callback=lambda batch_job: self._update_job_progress(
                job_id, batch_job, progress_callback
            ),
        )
        ...
        Thread(
            target=self._monitor_basic_search_completion,
    ...
    def _execute_comprehensive_search(
        ...
        # Monitor completion
        Thread(
            target=self._monitor_comprehensive_search_completion,
    ...
    def _execute_validation_search(
        ...
        Thread(
            target=self._monitor_validation_completion,
            args=(job_id, processing_job_id, job_info, progress_callback),
            daemon=True,
        ).start()
    def _execute_bulk_enhancement(
        ...
        processing_job_id = self.batch_processing_manager.submit_batch_job(
            ...
            processing_mode=ProcessingMode.INTELLIGENT,
            priority=BatchPriority.NORMAL,
            parameters={"comprehensive": True, "background_priority": "high"},
            callback=lambda batch_job: self._update_job_progress(
                job_id, batch_job, progress_callback
            ),
        )
        ...
        Thread(
            target=self._monitor_enhancement_completion,
            args=(job_id, processing_job_id, job_info, progress_callback),
            daemon=True,
        ).start()
    ...
    try:
            ...
            if status and status["status"] == "completed":
                ...
                results = self._process_engine_results(
                    raw_results, job_info["search_type"]
                )
    ...
    try:
            ...
            if status and status["status"] == "completed":
                ...
                raw_results = self.batch_processing_manager.get_job_results(
                    processing_job_id
                )
                results = self._process_comprehensive_results(
                    raw_results, job_info["search_type"]
                )
    ...
    try:
            ...
            if status and status["status"] == "completed":
                ...
                results = self._process_enhancement_results(
                    raw_results, job_info["search_type"]
                )
            ...
            with self.stats_lock:
                ...
                if total_time > 0:
                    ...
                    self.average_throughput = (
                        self.average_throughput * (self.total_jobs_processed - 1)
                        + current_throughput
    ...
    try:
            ...
            logger.info(
                f"Batch search job {job_id} completed: "
        ...
        with self.jobs_lock:
            if job_id in self.active_jobs:
                ...
                if "processing_job_id" in job_info:
                    self.batch_processing_manager.cancel_job(
                        job_info["processing_job_id"]
                    )
    ...
    try:
            ...
            with open(file_path, "w", newline="", encoding="utf-8") as csvfile:
                ...
                writer.writerow(
                    ["Total Time (s)", f"{summary.total_processing_time:.2f}"]
                )
                writer.writerow(
                    [
                        "Average Time per Item (s)",
                        f"{summary.average_time_per_item:.2f}",
                    ]
                )
                writer.writerow(["Total API Calls", summary.api_calls_total])

            logger.info(
                f"Exported batch search results for job {job_id} to {file_path}"
            )
            return True
    ...
    def get_integration_statistics(self) -> Dict[str, Any]:
        ...
        stats["batch_processing_manager"] = (
            self.batch_processing_manager.get_manager_statistics()
        )
