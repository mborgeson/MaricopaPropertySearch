...
class WebScrapingFallback:
    ...
    async def collect_tax_data_fallback(self, apn: str) -> Dict[str, Any]:
        ...
        result = {"tax_data_collected": False, "tax_records": [], "tax_errors": []}
        ...
        try:
            async with async_playwright() as p:
                ...
                await page.wait_for_selector(
                    'input[name*="apn"], input[id*="apn"], input[placeholder*="APN"]',
                    timeout=10000,
                )
                apn_input = await page.query_selector(
                    'input[name*="apn"], input[id*="apn"], input[placeholder*="APN"]'
                )
                ...
                if apn_input:
                    ...
                    if search_button:
                        ...
                        if tax_records:
                            ...
                            logger.info(
                                f"Collected {len(tax_records)} tax records via web scraping for APN: {apn}"
                            )
        ...
        try:
            async with async_playwright() as p:
                ...
                search_input = await page.query_selector(
                    'input[name*="search"], input[name*="apn"], input[id*="search"]'
                )
                ...
                if search_input:
                    ...
                    if search_button:
                        ...
                        if sales_records:
                            ...
                            logger.info(
                                f"Collected {len(sales_records)} sales records via web scraping for APN: {apn}"
                            )
...
class BackgroundDataWorker(QThread):
    ...
    def _collect_data(self, job: DataCollectionJob) -> Dict[str, Any]:
        ...
        try:
            ...
            result = asyncio.run(
                self.data_collector.collect_property_data_progressive(apn)
            )
            ...
            logger.info(
                f"Data collection completed for APN {apn} in {processing_time:.2f}s"
            )
            return result_dict
...
class UnifiedDataCollector:
    ...
    async def collect_property_data_progressive(
        self, apn: str, callback=None
    ) -> ProgressiveResults:
        """
        ...
        try:
            ...
            logger.info(
                f"Stage 1 (Basic) completed in {stage_time:.3f}s for APN: {apn}"
            )

            # STAGE 2: Detailed property data with fallback (Target: <2 seconds total)
            stage_start = time.time()
            detailed_data = await self._collect_detailed_data_with_fallback(
                apn, basic_data
            )
            stage_time = time.time() - stage_start
            ...
            logger.info(
                f"Stage 2 (Detailed) completed in {stage_time:.3f}s for APN: {apn}"
            )

            # STAGE 3: Extended data with comprehensive fallback (Target: <3 seconds total)
            stage_start = time.time()
            extended_data = await self._collect_extended_data_with_fallback(
                apn, results.data
            )
            stage_time = time.time() - stage_start
            ...
            logger.info(
                f"Stage 3 (Extended) completed in {stage_time:.3f}s for APN: {apn}"
            )
            ...
            logger.info(
                f"Unified progressive collection completed in {results.collection_time:.3f}s for APN: {apn}"
            )
    ...
    async def _collect_basic_data_fast(self, apn: str) -> Dict[str, Any]:
        ...
        if basic_data:
            ...
            return {"data_collection_stage": "basic", "basic_data_available": False}

    async def _collect_detailed_data_with_fallback(
        self, apn: str, basic_data: Dict
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if detailed_data:
                ...
                if "valuations" in detailed_data and detailed_data["valuations"]:
                    result["valuation_summary"] = self._process_valuations_fast(
                        detailed_data["valuations"]
                    )
                ...
                if (
                    ...
                    result["property_summary"] = self._process_residential_fast(
                        detailed_data["residential_details"]
                    )
            else:
    ...
    async def _collect_extended_data_with_fallback(
        self, apn: str, existing_data: Dict
    ) -> Dict[str, Any]:
        ...
        if "sales_history" not in existing_data:
            api_tasks.append(
                asyncio.create_task(
                    self._get_sales_history_fast(apn), name="sales_history"
                )
            )
            ...
            try:
                ...
                for i, result in enumerate(completed_tasks):
                    ...
                    if not isinstance(result, Exception) and result:
                        ...
                        logger.debug(
                            f"API extended data failed: {task_name} - {result}"
                        )

                # If API completely failed for tax and sales, try web scraping fallback
                if not api_success or (
                    "tax_info" not in extended_data
                    and "sales_history" not in extended_data
                ):
                    logger.info(
                        f"API methods insufficient, trying web scraping fallback for APN: {apn}"
                    )
                    ...
                    if "tax_info" not in extended_data:
                        try:
                            ...
                            if tax_fallback.get("tax_data_collected"):
                                ...
                                extended_data["extended_data_available"] = True
                                extended_data["fallback_usage"].append(
                                    "tax_web_scraping"
                                )
                                self.collection_stats["fallback_usage"][
                                    "web_scraping"
                                ] += 1
                                logger.info(
                    ...
                    if "sales_history" not in extended_data:
                        try:
                            ...
                            if sales_fallback.get("sales_data_collected"):
                                extended_data["sales_history"] = sales_fallback[
                                    "sales_records"
                                ]
                                extended_data["extended_data_available"] = True
                                extended_data["fallback_usage"].append(
    ...
    def _update_performance_stats(self, collection_time: float):
        ...
        self.collection_stats["avg_collection_time"] = (
            prev_avg * (count - 1) + collection_time
        ) / count
    ...
    def collect_data_for_apn_sync(self, apn: str) -> Dict[str, Any]:
        ...
        """
        logger.info(
            f"Starting comprehensive multi-script data collection for APN: {apn}"
        )
        ...
        logger.info(
            f"Multi-script execution completed for APN {apn}: {successful_scripts}/{total_scripts} scripts successful"
        )

        if comprehensive_results["fallback_usage"]:
            logger.info(
                f"Fallback methods used: {', '.join(comprehensive_results['fallback_usage'])}"
            )
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        for stage, times in self.collection_stats["stage_times"].items():
            stage_averages[f"{stage}_avg_time"] = (
                sum(times) / max(len(times), 1) if times else 0.0
            )
...
class BackgroundDataCollectionManager(QObject):
    ...
    def start_collection(self):
        ...
        self.worker = BackgroundDataWorker(
            self.unified_collector, self.max_concurrent_jobs
        )
