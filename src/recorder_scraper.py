...
class MaricopaRecorderScraper:
    ...
    def scrape_document_data_for_apn(self, apn: str, playwright_page) -> Optional[Dict[str, Any]]:
        """
    ...
    def _search_by_parcel(self, apn: str, page) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            for selector in selectors_to_try:
                try:
                    ...
                    if parcel_input.count() > 0:
                        ...
                        parcel_filled = True
                        logger.debug(f"Successfully filled parcel input using selector: {selector}")
                        break
                except Exception as e:
            ...
            for selector in search_selectors:
                try:
                    ...
                    if search_button.count() > 0:
                        search_button.click()
                        search_clicked = True
                        logger.debug(f"Successfully clicked search using selector: {selector}")
                        break
                except Exception as e:
            ...
            try:
                ...
                logger.warning(f"Page load timeout for APN {apn}, attempting to extract results anyway: {e}")
        ...
        try:
            ...
            try:
                if documents:
                    ...
                    logger.debug(f"Extracted {len(sales_history)} sales records for APN {apn}")
            except Exception as e:
            ...
            try:
                if documents:
                    ...
                    logger.debug(f"Extracted {len(transfers)} transfer records for APN {apn}")
            except Exception as e:
                logger.warning(f"Error extracting property transfers for APN {apn}: {e}")
            ...
            logger.info(f"Document extraction completed for APN {apn}: {len(documents)} docs, {len(sales_history)} sales, {len(transfers)} transfers")
            return result
    ...
    def _parse_document_table(self, content: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for pattern in row_patterns:
                ...
                if documents:  # If we found documents with this pattern, stop trying others
                    break
    ...
    def _generic_table_parse(self, content: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            td_pattern = r'<td[^>]*>(.*?)</td>'
            table_sections = re.findall(r'<table[^>]*>(.*?)</table>', content, re.DOTALL | re.IGNORECASE)
                ...
                for row in rows[1:]:  # Skip header row
                    ...
                    if len(cells) >= 3:
                        ...
                        document = {
                            ...
                            'date': self._parse_date(clean_cells[1]) if len(clean_cells) > 1 else None,
                            'document_type': clean_cells[2] if len(clean_cells) > 2 else 'UNKNOWN',
                            'description': clean_cells[3] if len(clean_cells) > 3 else '',
    ...
    def _extract_sales_from_documents(self, documents: List[Dict]) -> List[Dict[str, Any]]:
        """Extract sales transactions from document records"""
        sales_history = []
        
        # Document types that typically indicate sales
        sales_doc_types = [
            'DEED', 'GRANT DEED', 'WARRANTY DEED', 'QUIT CLAIM DEED',
            'SPECIAL WARRANTY DEED', 'TRUSTEE DEED', 'SHERIFF DEED',
            'BARGAIN AND SALE DEED', 'DEED OF TRUST'
        ]
        ...
        try:
            for doc in documents:
                ...
                is_sale_doc = any(sale_type in doc_type for sale_type in sales_doc_types)
                ...
                if is_sale_doc:
                    sale_record = {
                        ...
                        'sale_price': self._extract_sale_price(doc.get('description', '')),
                        'seller_name': self._extract_grantor(doc.get('description', '')),
    ...
    def _extract_property_transfers(self, documents: List[Dict]) -> List[Dict[str, Any]]:
        """Extract all property transfer documents"""
        transfers = []
        
        transfer_doc_types = [
            'DEED', 'TRANSFER', 'CONVEYANCE', 'ASSIGNMENT',
            'CERTIFICATE', 'PATENT', 'FORECLOSURE'
        ]
        ...
        try:
            for doc in documents:
                ...
                is_transfer = any(transfer_type in doc_type for transfer_type in transfer_doc_types)
                ...
                if is_transfer:
                    transfer = {
                        ...
                        'consideration': self._extract_sale_price(doc.get('description', ''))
                    }
    ...
    def search_documents_by_name(self, name: str, playwright_page) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            name_input = playwright_page.locator('input[name*="name"], input[id*="name"], input[placeholder*="name" i]').first
            if name_input.count() > 0:
                name_input.fill(name)
            
            # Click search
            search_button = playwright_page.locator('input[type="submit"], button[type="submit"], button:has-text("Search")').first
            search_button.click()
    ...
    def format_recorder_data_for_database(self, recorder_data: Dict[str, Any]) -> Dict[str, Any]:
