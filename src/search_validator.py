...
class SearchValidator:
    ...
    def validate_search_input(
        ...
        if search_type == SearchType.APN:
            ...
            is_valid, type_errors, type_warnings = self._validate_owner_name(
                sanitized_input
            )
        elif search_type == SearchType.ADDRESS:
            is_valid, type_errors, type_warnings = self._validate_address(
                sanitized_input
            )
        else:
        ...
        if not is_valid:
            logger.warning(
                f"Validation failed for {search_type.value}: {original_input} -> {errors}"
            )
        elif warnings:
            logger.info(
                f"Validation warnings for {search_type.value}: {original_input} -> {warnings}"
            )

        return ValidationResult(
            is_valid, sanitized_input, errors, warnings, search_type
        )
    ...
    def _validate_apn(self, apn: str) -> Tuple[bool, List[str], List[str]]:
        ...
        if not apn_valid:
            ...
            if re.match(r"^\d{6,15}[A-Z]?$", cleaned_apn):
                ...
                errors.append(
                    "Invalid APN format. Expected format: XXX-XX-XXXX or similar"
                )
    ...
    def auto_detect_search_type(self, input_str: str) -> Optional[SearchType]:
        ...
        if re.search(r"^\d+\s", sanitized) and re.search(
            r"\b(ST|STREET|AVE|AVENUE|RD|ROAD|BLVD|DR|LN|CT|PL|WAY|CIR)\b",
            sanitized,
            re.IGNORECASE,
        ):
        ...
        if (
            re.match(r"^[A-Za-z\s\.\-\'&,]+$", sanitized)
            and len(sanitized.split()) >= 1
        ):
        ...
        if re.match(r"^\d", partial_input):
            if "-" in partial_input:
                ...
                if len(partial_input) >= 3:
                    suggestions["apn"].append(
                        f"{partial_input[:3]}-{partial_input[3:]}"
                    )
