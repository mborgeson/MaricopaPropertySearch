...
def setup_test_tables():
    ...
    try:
        ...
        cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_test_properties_apn ON {DB_CONFIG['test_schema']}.properties(apn)")
        cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_test_properties_owner ON {DB_CONFIG['test_schema']}.properties(owner_name)")
        cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_test_properties_address ON {DB_CONFIG['test_schema']}.properties(property_address)")
        cursor.execute(f"CREATE INDEX IF NOT EXISTS idx_test_properties_city ON {DB_CONFIG['test_schema']}.properties(city)")
...
def load_test_data():
    ...
    try:
        ...
        cursor.execute(f"TRUNCATE TABLE {DB_CONFIG['test_schema']}.sales_history CASCADE") 
        cursor.execute(f"TRUNCATE TABLE {DB_CONFIG['test_schema']}.documents CASCADE")
        cursor.execute(f"TRUNCATE TABLE {DB_CONFIG['test_schema']}.search_history CASCADE")
        cursor.execute(f"TRUNCATE TABLE {DB_CONFIG['test_schema']}.properties RESTART IDENTITY CASCADE")
...
def validate_test_setup():
    ...
    try:
        ...
        cursor.execute(f"SELECT apn, owner_name, property_address FROM {DB_CONFIG['test_schema']}.properties WHERE apn = '301-07-042'")
...
def get_test_config_info():
    ...
    print(f"Connection String: postgresql://{DB_CONFIG['user']}:***@{DB_CONFIG['host']}:{DB_CONFIG['port']}/{DB_CONFIG['database']}")
...
def main():
    ...
    print(f"✓ Test schema: {DB_CONFIG['test_schema']} in database {DB_CONFIG['database']}")
