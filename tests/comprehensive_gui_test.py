...
class GUITestFramework:
    ...
    def test_component_initialization(self):
        ...
        try:
            ...
            assert hasattr(self.main_window, 'setWindowTitle'), "Window title method missing"
            assert hasattr(self.main_window, 'setGeometry'), "Window geometry method missing"

            # Check core component attributes
            required_attributes = [
                'db_manager', 'search_engine', 'background_manager', 'batch_manager',
                'results_table', 'search_input', 'status_bar', 'notification_area'
            ]
        ...
        try:
            ...
            if hasattr(self.main_window, 'setup_ui'):
                ...
                try:
                    ...
                    self.test_results['setup_ui_execution'] = "✅ PASS - Executes without error"
                except Exception as e:
                    self.test_results['setup_ui_execution'] = f"⚠️ WARNING - Error during setup: {e}"
            else:
    ...
    def test_search_components(self):
        ...
        try:
            ...
            if hasattr(self.main_window, 'search_input') and self.main_window.search_input:
                self.test_results['search_input'] = "✅ PASS - Search input exists"

                # Test placeholder text
                placeholder = self.main_window.search_input.placeholderText()
                if placeholder:
                    self.test_results['search_placeholder'] = f"✅ PASS - Placeholder: '{placeholder}'"
                else:
                    self.test_results['search_placeholder'] = "⚠️ WARNING - No placeholder text"
            else:
            ...
            if hasattr(self.main_window, 'search_btn') and self.main_window.search_btn:
                ...
                if button_text:
                    self.test_results['search_button_text'] = f"✅ PASS - Button text: '{button_text}'"
                else:
                    self.test_results['search_button_text'] = "⚠️ WARNING - No button text"
            else:
            ...
            if hasattr(self.main_window, 'perform_search'):
                self.test_results['perform_search_method'] = "✅ PASS - Search method exists"
            else:
                self.test_results['perform_search_method'] = "❌ FAIL - Search method missing"
    ...
    def test_results_table(self):
        ...
        try:
            ...
            if hasattr(self.main_window, 'results_table') and self.main_window.results_table:
                ...
                self.test_results['table_columns'] = f"✅ INFO - Column count: {column_count}"
    ...
    def test_tab_widgets(self):
        ...
        try:
            ...
            if tab_widgets:
                self.test_results['tab_widgets_found'] = f"✅ PASS - Found {len(tab_widgets)} tab widget(s)"

                for i, tab_widget in enumerate(tab_widgets):
                    tab_count = tab_widget.count()
                    self.test_results[f'tab_widget_{i}_count'] = f"✅ INFO - Tab widget {i} has {tab_count} tabs"
                    ...
                    self.test_results[f'tab_widget_{i}_names'] = f"✅ INFO - Tab names: {tab_names}"
            else:
                self.test_results['tab_widgets_found'] = "⚠️ WARNING - No tab widgets found"
        ...
        try:
            ...
            for method_name in data_collection_methods:
                if hasattr(self.main_window, method_name):
                    self.test_results[f'{method_name}_method'] = "✅ PASS - Method exists"
                else:
                    self.test_results[f'{method_name}_method'] = "❌ FAIL - Method missing"
            ...
            if hasattr(self.main_window, 'background_manager'):
                if self.main_window.background_manager:
                    self.test_results['background_manager'] = "✅ PASS - Background manager exists"
                else:
                    self.test_results['background_manager'] = "⚠️ WARNING - Background manager is None"
            else:
                self.test_results['background_manager'] = "❌ FAIL - Background manager attribute missing"
    ...
    def test_menu_and_toolbar(self):
        ...
        try:
            ...
            if toolbars:
                self.test_results['toolbars'] = f"✅ PASS - Found {len(toolbars)} toolbar(s)"
            else:
    ...
    def test_signal_connections(self):
        ...
        try:
            ...
            if hasattr(self.main_window, 'connect_signals'):
                self.test_results['connect_signals_method'] = "✅ PASS - connect_signals method exists"

                # Try to call it (may already be called)
                try:
                    self.main_window.connect_signals()
                    self.test_results['connect_signals_execution'] = "✅ PASS - Executes without error"
                except Exception as e:
                    self.test_results['connect_signals_execution'] = f"⚠️ WARNING - Error: {e}"
            else:
                self.test_results['connect_signals_method'] = "❌ FAIL - connect_signals method missing"
            ...
            if hasattr(self.main_window, 'search_input') and self.main_window.search_input:
                ...
                try:
                    ...
                    self.test_results['search_input_signals'] = "✅ INFO - Search input available for signal testing"
                except Exception as e:
                    self.test_results['search_input_signals'] = f"⚠️ WARNING - Signal test error: {e}"
        ...
        try:
            ...
            for method_name in error_handling_methods:
                if hasattr(self.main_window, method_name):
                    ...
                    self.test_results[f'{method_name}'] = "⚠️ INFO - Method not found (may use generic error handling)"

            if found_methods > 0:
                self.test_results['error_handling_overall'] = f"✅ PASS - Found {found_methods} error handling methods"
            else:
                self.test_results['error_handling_overall'] = "⚠️ WARNING - No specific error handling methods found"
