...
class TestMissouriAvenueProperty:
    ...
    def test_address_search_basic_functionality(self, app_instance, db_manager, missouri_property_data):
        ...
        for row in range(main_window.results_table.rowCount()):
            for col in range(main_window.results_table.columnCount()):
                ...
                if item:
                    assert "Not Available" not in item.text(), f"Found 'Not Available' in table at ({row}, {col})"
    
    def test_auto_collection_feature_missouri_ave(self, app_instance, db_manager, missouri_property_data):
    ...
    def test_data_availability_missouri_ave(self, app_instance, db_manager, missouri_property_data):
    ...
    def test_database_integration_missouri_ave(self, app_instance, db_manager, missouri_property_data):
    ...
    def test_manual_vs_auto_collection_missouri_ave(self, app_instance, db_manager, missouri_property_data):
    ...
    @pytest.mark.asyncio
    async def test_background_collection_performance_missouri_ave(self, app_instance, db_manager, missouri_property_data):
...
class TestUXMessageImprovements:
    ...
    def test_no_not_available_messages_in_ui(self, app_instance, db_manager):
        ...
        assert len(not_available_found) == 0, f"Found 'Not Available' messages: {not_available_found}"
    ...
    def test_actionable_messages_present(self, app_instance, db_manager):
        ...
        found_actionable = any(phrase in all_text_combined for phrase in actionable_phrases)
        assert found_actionable, f"No actionable messages found. Text: {all_text_combined[:500]}"
    ...
    def test_tooltip_hover_messages(self, app_instance, db_manager):
        ...
        for element in tooltip_elements:
            if element.isEnabled():
                ...
                assert len(tooltip) > 10, f"Element {element.text()} missing helpful tooltip"
