...
class TestRunner:
    ...
    def _parse_junit_xml(self, xml_file: Path) -> dict:
        ...
        try:
            ...
            return {
                ...
                "tests_passed": int(root.get("tests", 0))
    ...
    def _generate_final_report(self) -> dict:
        ...
        overall_success = (
            suites_passed == suites_total and total_failed == 0 and total_errors == 0
        )
    ...
    def _generate_recommendations(self) -> list:
            ...
            if not results.get("success", False):
                if results.get("tests_failed", 0) > 0:
                    recommendations.append(
                        f"Fix {results['tests_failed']} failing tests in {suite_name} suite"
                    )

                if results.get("tests_errors", 0) > 0:
                    recommendations.append(
                        f"Resolve {results['tests_errors']} test errors in {suite_name} suite"
                    )

                if results.get("execution_time", 0) > TEST_SUITES.get(
                    suite_name, {}
                ).get("timeout", 3600):
                    recommendations.append(
                        f"Optimize {suite_name} test performance - execution time exceeded timeout"
                    )
        ...
        if performance_results.get("tests_failed", 0) > 0:
            recommendations.append(
                "Performance benchmarks not met - review application optimization"
            )

        # Coverage recommendations
        if len(self.test_results) < 3:
            recommendations.append(
                "Run complete test suite including integration and e2e tests"
            )
    ...
    def _print_final_summary(self, report: dict):
        ...
        print(f"{'='*80}")
        print(
            f"Overall Result: {'✅ PASSED' if report['overall_success'] else '❌ FAILED'}"
        )
...
def main():
    ...
    parser.add_argument(
        "--suite", choices=list(TEST_SUITES.keys()), help="Run specific test suite"
    )
    parser.add_argument("--smoke", action="store_true", help="Run smoke tests only")
    parser.add_argument(
        "--baseline", action="store_true", help="Establish performance baseline"
    )
    parser.add_argument("--all", action="store_true", help="Run all test suites")
    parser.add_argument("--verbose", action="store_true", help="Verbose output")
    parser.add_argument(
        "--no-capture", action="store_true", help="Don't capture stdout"
    )
    parser.add_argument(
        "--pdb", action="store_true", help="Drop into debugger on failures"
    )
    parser.add_argument(
        "--maxfail", type=int, default=None, help="Stop after N failures"
    )
    ...
    try:
        if args.smoke:
            ...
            results = runner.run_comprehensive_tests(
                list(TEST_SUITES.keys()), **pytest_args
            )
        else:
            # Default: run core test suites
            results = runner.run_comprehensive_tests(
                ["unit", "integration"], **pytest_args
            )
