...
class MockDataGenerator:
    ...
    def generate_property_data(self, count: int = 10) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            ...
            property_data["market_value"] = int(
                property_data["assessed_value"] * random.uniform(1.05, 1.25)
            )
    ...
    def generate_sales_history(
        ...
        for i, sale_date in enumerate(sale_dates):
            ...
            appreciation_factor = 1 + (len(sale_dates) - i - 1) * random.uniform(
                0.03, 0.08
            )
                ...
                "data": {
                    ...
                    "additional_info": {
                        ...
                        "school_district": random.choice(
                            ["Phoenix Union", "Scottsdale Unified", "Tempe Elementary"]
                        ),
...
class MockResponseProvider:
    ...
    def get_search_response(self, scenario: str, **kwargs) -> Dict[str, Any]:
        ...
        if scenario not in self.response_cache:
            self.response_cache[scenario] = self._generate_scenario_response(
                scenario, **kwargs
            )
        ...
        if scenario == "successful_search":
            ...
            for prop in properties:
                if search_type == "owner":
                    prop["owner_name"] = (
                        f"{search_term}, {random.choice(['JOHN', 'MARY', 'ROBERT', 'LISA'])}"
                    )
                elif search_type == "address":
                    prop["property_address"] = (
                        f"{random.randint(100, 9999)} {search_term}"
                    )
