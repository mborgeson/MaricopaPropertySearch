...
class TestProfessionalAppearance:
    ...
    def test_search_interface_clarity(self, qt_app, app_config):
        ...
        search_types = [window.search_type_combo.itemText(i) 
    ...
    def test_loading_states_professional(self, qt_app, app_config, performance_timer):
        ...
        with patch.object(window, '_perform_search', side_effect=mock_slow_search):
            ...
            if hasattr(window, 'progress_bar'):
                assert window.progress_bar.isVisible(), "Progress bar should be visible during search"
...
class TestUserWorkflowExperience:
    ...
    def test_new_user_first_search_experience(self, qt_app, app_config, test_database):
        ...
        assert window.search_type_combo.currentText() in ["Owner Name", "Property Address", "APN"]
        ...
        def mock_search():
            ...
            original_results = [
                {"apn": "101-01-001A", "owner_name": "SMITH, JOHN", "property_address": "123 MAIN ST"}
            ]
            search_completed = True
    ...
    def test_power_user_rapid_search_experience(self, qt_app, app_config, test_database):
        ...
        for search_type, term in search_terms:
            ...
            assert search_time < 1.0, f"Individual search took {search_time:.3f}s, should be < 1s"
            
        # Overall rapid search session should be efficient
        average_time = total_time / len(search_terms)
        assert average_time < 0.8, f"Average search time {average_time:.3f}s too slow for power users"
    ...
    def test_error_recovery_user_experience(self, qt_app, app_config, network_simulator):
        ...
        with network_simulator.apply_condition('offline'):
            ...
            with patch.object(window, '_perform_search', side_effect=mock_failing_search):
                window.search_btn.click()
                qt_app.processEvents()
                ...
                if hasattr(window, 'statusBar'):
                    ...
                    assert "network" in status_text.lower() or "connection" in status_text.lower()
...
class TestDataDisplayExperience:
    ...
    def test_search_results_presentation(self, qt_app, app_config, sample_search_results):
        ...
        for field in important_fields:
            assert any(field.lower() in header.lower() for header in headers), f"Missing important field: {field}"
        ...
        if table.rowCount() > 0:
            first_row_data = [table.item(0, col).text() 
    ...
    def test_background_enhancement_indication(self, qt_app, app_config):
        ...
        if hasattr(window, 'status_label') or hasattr(window, 'statusBar'):
            ...
            def mock_enhancement():
                if hasattr(window, 'statusBar'):
                    window.statusBar().showMessage("Enhancing results with additional data...")
                qt_app.processEvents()
                time.sleep(0.1)
                if hasattr(window, 'statusBar'):
            ...
            if hasattr(window, 'statusBar'):
                ...
                assert len(final_message) > 0, "Status should indicate enhancement completion"
...
class TestResponsivenessExperience:
    ...
    def test_large_result_set_display_performance(self, qt_app, app_config):
        ...
        qt_app.processEvents()
        assert window.search_input.text() == "test", "UI should remain responsive after large data load"
