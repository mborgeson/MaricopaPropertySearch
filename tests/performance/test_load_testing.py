...
class TestConcurrentUserLoad:
    ...
    def test_single_user_baseline_performance(
        ...
        assert (
            avg_response < 1.0
        ), f"Average response time {avg_response:.3f}s should be < 1s"
        assert (
            max_response < 2.0
        ), f"Max response time {max_response:.3f}s should be < 2s"
        assert (
            percentile_95 < 1.5
        ), f"95th percentile {percentile_95:.3f}s should be < 1.5s"
    ...
    def test_concurrent_database_access(self, test_database, app_config):
        ...
        for user_count in CONCURRENT_USERS:
            ...
            if all_response_times:
                ...
                percentile_95 = sorted(all_response_times)[
                    int(len(all_response_times) * 0.95)
                ]
                error_rate = (
                    total_errors / (len(all_response_times) + total_errors)
                ...
                assert (
                    avg_response < max_avg_response
                ), f"Average response {avg_response:.3f}s too high for {user_count} users"
                assert (
                    percentile_95 < max_95th_percentile
                ), f"95th percentile {percentile_95:.3f}s too high for {user_count} users"
                assert (
    ...
    def test_memory_usage_under_load(self, test_database, app_config):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(memory_intensive_search, 20) for _ in range(5)]
    ...
    def test_connection_pool_under_load(self, test_database, app_config):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=20) as executor:
            futures = [
                executor.submit(stress_connections, user_id, 5) for user_id in range(20)
            ]
...
class TestScalabilityLimits:
    ...
    def test_sustained_load_performance(self, test_database):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(sustained_search_load) for _ in range(10)]
        ...
        if all_response_times:
            ...
            assert (
                degradation < 1.5
            ), f"Performance degraded {degradation:.2f}x over time"
    ...
    def test_resource_exhaustion_handling(self, test_database, app_config):
        ...
        def resource_intensive_operation(operation_id):
            ...
            try:
                ...
                processed = [
                    {"processed_data": str(item).upper(), "length": len(str(item))}
